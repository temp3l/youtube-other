...
await execute("pnpm", [
  "exec", "vitest", "run", "-c", "vitest.unit.config.ts", "--bail=1", ...testFiles,
...
const invariantTestSummary = {
  ...
  invariants: {
    ...
    shadowCandidateProjection: "pass",
    explicitPropositionBoundary: "pass",
...
const payloads = {
  ...
  "test-summary.json": `${JSON.stringify({
    command: `pnpm exec vitest run -c vitest.unit.config.ts --bail=1 ${testFiles.join(" ")}`,
    result: "pass",
    testCount: vitestResult.numTotalTests,
