...
describe("SaaS runtime", () => {
  ...
  it("keeps project, typed episode, workflow, and stale-edit actions behind the BFF", async () => {
    ...
    const journey: SaasJourneyGateway = {
      ...
      bulk: {
        preflight: async () => ({ id: "batch-1" }),
        get: async (_identity, batchId) => ({ id: batchId, status: "ready", selectionFingerprint: "e".repeat(64), items: [{ id: "item-1", status: "eligible", reasons: [], eligible: true }] }),
        launch: async () => undefined,
        retry: async () => undefined,
        cancel: async (_identity, batchId) => { cancelledBulkBatches.push(batchId); },
      },
    ...
    expect(settingsHtml).toContain("German");
    expect(settingsHtml).toContain("Speech settings — history");
    expect(settingsHtml).toContain("Voice assignment and consent are resolved server-side");
    ...
    const bulkCancelled = await fetch(`${baseUrl}/bulk/batch-1:cancel`, { method: "POST", redirect: "manual", headers: { origin: baseUrl, "content-type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ idempotencyKey: "bulk-cancel-1" }) });
    expect(bulkCancelled.headers.get("location")).toBe("/bulk?batch=batch-1");
    expect(cancelledBulkBatches).toEqual(["batch-1"]);
