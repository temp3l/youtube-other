...
async function renderJourneyPage(identity: SaasIdentity, path: string, gateway: SaasJourneyGateway, search = "", publicationExecutionEnabled = false, pendingInvalidations?: ReadonlyMap<string, PendingInvalidation>): Promise<string | null> {
  try {
    ...
    if (path === "/actions") {
      ...
      const activeSince = Date.now() - 30 * 24 * 60 * 60 * 1_000;
      const auditRows = (await gateway.listAudit(identity)).items
        .filter((event) => Number.isFinite(Date.parse(event.occurredAt)) && Date.parse(event.occurredAt) >= activeSince)
        .map((event) => `<div class="row"><div><strong>${escapeHtml(event.action)}</strong><span>${escapeHtml(event.occurredAt)} · correlation ${escapeHtml(event.correlationId)}</span></div><span class="tag neutral">Recorded</span></div>`);
      return shell(identity.session, path, "Action center", "Current server-authoritative actions and recent tenant audit facts. Immutable audit retention remains separate.", `<section class="card"><h2>Active actions</h2><p>Actions deep-link to current episode state and are recalculated on every request.</p></section><div class="stack" style="margin-top:12px">${rows.length ? rows.join("") : empty("No active actions", "Current workflow, review, and production actions will appear here when the server enables them.")}</div><section class="card" style="margin-top:18px"><h2>Recent domain events</h2><p>Tenant-scoped audit facts from the last 30 days. Expired actions and raw audit payloads are not shown here.</p></section><div class="stack" style="margin-top:12px">${auditRows.length ? auditRows.join("") : empty("No recent domain events", "New tenant-scoped facts will appear here when recorded.")}</div>`);
    }
    if (path === "/bulk") {
