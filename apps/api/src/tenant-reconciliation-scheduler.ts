...
function publicationIntent(value: unknown): PublicationIntent {
  if (
  ...
  return value as PublicationIntent;
}
...
function createTenantOutboxRepository(input: {
  ...
  return {
    claimNextOutbox: (request) => input.repository.withWorkspaceTransaction(
      input.workspaceId,
      (transaction) => transaction.claimNextOutbox(request)
    ),
    markOutboxDelivered: (request) => input.repository.withWorkspaceTransaction(
      input.workspaceId,
      (transaction) => transaction.markOutboxDelivered(request)
    ),
    rescheduleOutbox: (request) => input.repository.withWorkspaceTransaction(
      input.workspaceId,
...
export function createPostgresTenantYoutubeReconciliationScheduler(input: {
  ...
  const outbox = new DurableOutboxWorker(
    createTenantOutboxRepository({ repository, workspaceId: input.workspaceId }),
    {
      dispatch: async (event) => {
        if (event.workspaceId !== input.workspaceId || event.topic !== reconciliationTopic)
          throw new Error("Tenant reconciliation scheduler received an unauthorized event.");
        await worker.reconcile(publicationIntent(event.payload));
      },
    },
    {
      ...
      retryAt: input.retryAt ?? ((attempt, now) => new Date(now.getTime() + Math.min(60_000, 1_000 * 2 ** attempt))),
...
export function createLivePostgresTenantYoutubeReconciliationScheduler(input: Omit<
