...
export function createPostgresApiWorkflowAdmissionHandler(pool: PostgresPool): WorkflowAdmissionHandler {
...
export function createPostgresApiServer(input: {
  ...
  readonly options?: Omit<Parameters<typeof createApiServer>[0], "workflowAdmissionHandler">;
}) {
  return createApiServer({
    ...input.options,
    workflowAdmissionHandler: createPostgresApiWorkflowAdmissionHandler(input.pool),
  });
}
...
export async function startApiServer(port = 3333) {
  ...
  if (!config.workflowDatabaseUrl)
    throw new Error("MEDIAFORGE_WORKFLOW_DATABASE_URL is required to start the API.");
