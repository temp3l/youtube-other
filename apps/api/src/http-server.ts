...
export interface ApiUseCases {
  ...
  listAuditEvents(after: string | undefined, size: number, context: Required<Pick<ApiRequestContext, "workspaceId" | "requestId">>): Promise<{ readonly items: readonly ApiAuditEvent[]; readonly nextAfter?: string }>;
  ...
  getEpisode(episodeId: string, context: Required<Pick<ApiRequestContext, "workspaceId" | "projectId" | "requestId">>): Promise<{ readonly id: string; readonly revision: number; readonly content: unknown } | null>;
  replaceEpisodeContent(episodeId: string, input: EpisodeInput, context: Required<Pick<ApiRequestContext, "workspaceId" | "projectId" | "principal" | "requestId" | "ifMatch">>): Promise<{ readonly id: string; readonly revision: number; readonly content: EpisodeInput["content"] }>;
  admitWorkflow(input: WorkflowAdmission, context: Required<Pick<ApiRequestContext, "workspaceId" | "projectId" | "episodeId" | "principal" | "requestId" | "idempotencyKey">>): Promise<{ readonly workflowRunId: string; readonly jobId: string; readonly revision: number }>;
  ...
  listWorkflowSteps(runId: string, context: Required<Pick<ApiRequestContext, "workspaceId" | "projectId" | "requestId">>): Promise<{ readonly items: readonly unknown[] }>;
  cancelWorkflow(runId: string, context: Required<Pick<ApiRequestContext, "workspaceId" | "projectId" | "principal" | "requestId" | "ifMatch">>): Promise<{ readonly workflowRunId: string; readonly jobId: string; readonly revision: number }>;
  resumeWorkflow(runId: string, context: Required<Pick<ApiRequestContext, "workspaceId" | "projectId" | "principal" | "requestId" | "ifMatch" | "idempotencyKey">>): Promise<{ readonly workflowRunId: string; readonly jobId: string; readonly revision: number }>;
...
export function createDirectoryBackedRequestAuthenticator(input: {
  ...
  readonly directory: {
    findActive(workspaceId: string, oidcSubject: string): Promise<{
...
export function createApiWorkflowAdmissionUseCase(handler: Pick<WorkflowAdmissionHandler, "execute">, deadlineMs = 30_000): ApiUseCases["admitWorkflow"] {
  return async (input, context) => {
    ...
    const scopedIdempotencyKey = `v1:${crypto.createHash("sha256").update(JSON.stringify({
    ...
    const execution = createApplicationExecutionContext({
      context: {
        ...
        authorization: { decision: "allowed", requiredPermissions: ["workflow.start"] },
...
function strongIfMatch(request: IncomingMessage): string {
  ...
  if (!value) throw new ApplicationError("precondition_required", "If-Match is required.", false);
  if (!/^"(0|[1-9][0-9]*)"$/u.test(value))
    throw new ApplicationError("precondition_failed", "If-Match must contain one strong numeric ETag.", false);
  return value;
}
...
function requiredPermission(
  ...
  if (method === "POST" && matched.tail === "episodes")
    return "content.write";
  if (
    method === "GET" &&
    matched.episode &&
  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/health/live")
        return json(response, 200, { status: "ok" }, { "x-request-id": requestIdValue });
      if (request.method === "GET" && url.pathname === "/health/ready") {
        const ready = await options.readiness?.() ?? false;
        return json(response, ready ? 200 : 503, { status: ready ? "ready" : "unavailable" }, { "x-request-id": requestIdValue });
      }
      ...
      if (request.method === "GET" && !matched.project && matched.tail === "quota") { const result = await useCases.getQuota(context); if (!result) throw new ApplicationError("not_found", "Resource not found.", false); return json(response, 200, result, { etag: etag(result.revision), "x-request-id": requestIdValue }); }
      if (request.method === "GET" && !matched.project && matched.tail === "usage-records") { const result = await useCases.listUsageRecords(url.searchParams.get("page[after]") ?? undefined, pageSize(url), context); return json(response, 200, result, { "x-request-id": requestIdValue }); }
      if (request.method === "GET" && !matched.project && matched.tail === "audit-events") { const result = await useCases.listAuditEvents(url.searchParams.get("page[after]") ?? undefined, pageSize(url), context); return json(response, 200, result, { "x-request-id": requestIdValue }); }
      ...
      if (request.method === "POST" && matched.run && matched.runAction === "cancel" && matched.tail === `workflow-runs/${matched.run}:cancel`) { const match = ifMatch(request); if (!match) throw new ApplicationError("precondition_required", "If-Match is required.", false); const result = await useCases.cancelWorkflow(matched.run, { ...projectContext, ifMatch: match }); return json(response, 202, { ...result, links: { workflowRun: `/v1/workspaces/${matched.workspace}/projects/${matched.project}/workflow-runs/${result.workflowRunId}`, job: `/v1/workspaces/${matched.workspace}/projects/${matched.project}/jobs/${result.jobId}` } }, { location: `/v1/workspaces/${matched.workspace}/projects/${matched.project}/jobs/${result.jobId}`, "retry-after": "3", etag: etag(result.revision), "x-request-id": requestIdValue }); }
      if (request.method === "POST" && matched.run && matched.runAction === "resume" && matched.tail === `workflow-runs/${matched.run}:resume`) { const match = ifMatch(request); const key = idempotencyKey(request); if (!match || !key) throw new ApplicationError("precondition_required", "If-Match and Idempotency-Key are required.", false); const result = await useCases.resumeWorkflow(matched.run, { ...projectContext, ifMatch: match, idempotencyKey: key }); return json(response, 202, { ...result, links: { workflowRun: `/v1/workspaces/${matched.workspace}/projects/${matched.project}/workflow-runs/${result.workflowRunId}`, job: `/v1/workspaces/${matched.workspace}/projects/${matched.project}/jobs/${result.jobId}` } }, { location: `/v1/workspaces/${matched.workspace}/projects/${matched.project}/jobs/${result.jobId}`, "retry-after": "3", etag: etag(result.revision), "x-request-id": requestIdValue }); }
