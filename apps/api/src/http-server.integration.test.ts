...
const useCases: ApiUseCases = {
  ...
  getPublication: async () => ({
    ...
    artifactBindings: [
      { assetId: "asset-1", role: "video", contentHash: "content-hash" },
    ],
    channelId: "channel-1",
    visibility: "private",
    scheduledAt: null,
...
describe("HTTP API contract", () => {
  ...
  it("preserves stable operator-safe speech errors without exposing diagnostic causes", async () => {
    const speechUseCases = new Proxy(
      {},
      {
        get: () => async () => {
          throw Object.assign(
            new Error("Speech provider is temporarily unavailable."),
            {
    ...
    const running = await serve(
      createApiServer({
        useCases,
        speechUseCases: speechUseCases as never,
        authenticate,
        requestId: () => "request-speech-error",
      })
    );
    closers.push(running.close);
    const response = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/speech/estimates`,
      method: "POST",
  ...
  it("publishes a versioned OpenAPI document without local runtime details", async () => {
    const running = await serve(
      createApiServer({ useCases, authenticate, requestId: () => "request-1" })
    );
    closers.push(running.close);
    const response = await request({
      url: `${running.baseUrl}/v1/openapi.json`,
    });
    ...
    for (const requestInput of [
      {
        ...
        body: JSON.stringify({ reason: "Valid" }),
        status: 428,
      },
      {
        headers: {
      ...
      },
      {
        ...
        body: JSON.stringify({ reason: "x".repeat(2_001) }),
        status: 400,
      },
    ]) {
  ...
  it("maps workflow admission to one use case and returns asynchronous contract headers", async () => {
    ...
    const running = await serve(
      createApiServer({
        useCases: {
          ...useCases,
          admitWorkflow: async (input, context) => {
            admitted.push({ input, context });
            return { workflowRunId: "run-1", jobId: "job-1", revision: 0 };
          },
        },
        authenticate,
        requestId: () => "request-2",
    ...
    expect(JSON.parse(response.body) as unknown).toMatchObject({
      workflowRunId: "run-1",
      links: { workflowRun: expect.stringContaining("workflow-runs/run-1") },
    });
  ...
  it("maps episode, job, and workflow-step reads to their exact use cases", async () => {
    ...
    const running = await serve(
      createApiServer({
        useCases: {
          ...useCases,
          getEpisode: async (id, context) => {
            calls.push({ operation: "episode", id, context });
            return {
              id,
              revision: 5,
              content: { type: "dark_truth", version: "1" },
          ...
          listWorkflowSteps: async (id, context) => {
            calls.push({ operation: "steps", id, context });
            return { items: [{ id: "phase-render", status: "running" }] };
          },
        },
        authenticate,
    ...
    const episode = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/projects/project-1/episodes/episode-read`,
    });
    const job = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/projects/project-1/jobs/job-read`,
    });
    const steps = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-read/steps`,
    });
    ...
    expect(JSON.parse(steps.body) as unknown).toEqual({
      items: [{ id: "phase-render", status: "running" }],
    });
  ...
  it("enforces workflow control preconditions and returns asynchronous job links", async () => {
    ...
    const running = await serve(
      createApiServer({
        useCases: {
          ...useCases,
          cancelWorkflow: async (id, context) => {
            calls.push({ operation: "cancel", id, context });
            return { workflowRunId: id, jobId: "job-cancelled", revision: 7 };
          },
          resumeWorkflow: async (id, context) => {
            calls.push({ operation: "resume", id, context });
            return { workflowRunId: id, jobId: "job-resumed", revision: 8 };
          },
        },
        authenticate,
    ...
    const missingCancel = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1:cancel`,
      method: "POST",
    });
    const missingResume = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1:resume`,
      method: "POST",
      headers: { "if-match": '"6"' },
    });
    expect(missingCancel.status).toBe(428);
    expect(missingResume.status).toBe(428);
    expect(JSON.parse(missingCancel.body) as unknown).toMatchObject({
      code: "precondition_required",
    });
    expect(JSON.parse(missingResume.body) as unknown).toMatchObject({
      code: "precondition_required",
    });
    expect(calls).toEqual([]);
    ...
    const resumed = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1:resume`,
      method: "POST",
      headers: { "if-match": '"7"', "idempotency-key": "resume-key" },
    });
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        operation: "cancel",
        id: "run-1",
        context: expect.objectContaining({ ifMatch: '"6"' }),
      }),
      expect.objectContaining({
        operation: "resume",
        id: "run-1",
  ...
  it("returns RFC 9457 problems for preconditions and never runs the handler", async () => {
    const running = await serve(
      createApiServer({ useCases, authenticate, requestId: () => "request-3" })
    );
    closers.push(running.close);
    const response = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/projects/project-1/episodes/episode-1/workflow-runs`,
  ...
  it("returns an opaque not-found response for a foreign workspace", async () => {
    const running = await serve(
      createApiServer({ useCases, authenticate, requestId: () => "request-4" })
    );
    closers.push(running.close);
    const response = await request({
      url: `${running.baseUrl}/v1/workspaces/foreign/projects/project-1/validations`,
    });
    ...
    const workspaceResponse = await request({
      url: `${running.baseUrl}/v1/workspaces/foreign/quota`,
    });
    expect(workspaceResponse.status).toBe(404);
    expect(JSON.parse(workspaceResponse.body) as unknown).toMatchObject({
      code: "not_found",
    });
    ...
    expect(JSON.parse(episodeResponse.body) as unknown).toMatchObject({
      code: "not_found",
    });
  });
  ...
  it("maps workspace quota, usage, and audit reads without numeric precision loss", async () => {
              ...
              items: [
                {
                  ...
                  operation: "render",
                  unit: "frame",
                  quantityUnits: "9007199254740993",
                  costMinor: "42",
                  correctionOfUsageId: null,
                  attemptId: "attempt-1",
                  data: {},
                  occurredAt: "2026-08-01T12:00:00.000Z",
                },
              ],
              nextAfter: "signed-usage-cursor",
            };
          },
    ...
    const quota = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/quota`,
    });
    const usage = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/usage-records?page[size]=1&page[after]=cursor-1`,
    });
    const audit = await request({
      url: `${running.baseUrl}/v1/workspaces/ws-1/audit-events?page[size]=2`,
    });

    expect(quota.status).toBe(200);
    expect(quota.headers.etag).toBe('"7"');
    expect(JSON.parse(quota.body) as unknown).toMatchObject({
      budgetLimitMinor: "90071992547409930",
      availableMinor: "90071992547409900",
    });
    expect(JSON.parse(usage.body) as unknown).toMatchObject({
    ...
    const missingQuota = await serve(
      createApiServer({
    ...
    const missing = await request({
      url: `${missingQuota.baseUrl}/v1/workspaces/ws-1/quota`,
    });
    expect(missing.status).toBe(404);
    expect(JSON.parse(missing.body) as unknown).toMatchObject({
      code: "not_found",
    });
  });
  ...
  it("enforces the route-specific permission vocabulary before parsing command bodies", async () => {
    const running = await serve(
      createApiServer({
        ...
        authenticate: async (incoming) => ({
          ...
          workspaceId: "ws-1",
          permissions:
            typeof incoming.headers["x-test-permission"] === "string"
    ...
    const cases = [
      ...
      },
      {
        permission: "content.write",
        method: "POST",
        path: "/v1/workspaces/ws-1/projects/project-1/episodes",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          content: {
            type: "dark_truth",
      ...
      },
      {
        ...
        method: "GET",
        path: "/v1/workspaces/ws-1/projects/project-1/episodes/episode-1",
        status: 200,
      },
      {
        permission: "content.write",
      ...
      },
      {
        ...
        method: "POST",
        path: "/v1/workspaces/ws-1/projects/project-1/episodes/episode-1/workflow-runs",
        headers: {
          "content-type": "application/json",
          "idempotency-key": "admit-1",
        },
        body: workflowBody,
        status: 202,
      },
      {
        permission: "content.read",
        method: "GET",
        path: "/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1",
        status: 200,
      },
      {
        permission: "content.read",
        method: "GET",
        path: "/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1/steps",
        status: 200,
      },
      {
        permission: "workflow.cancel",
        method: "POST",
        path: "/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1:cancel",
        headers: { "if-match": '"2"' },
        status: 202,
      },
      {
        permission: "workflow.start",
        method: "POST",
        path: "/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1:resume",
        headers: { "if-match": '"2"', "idempotency-key": "resume-1" },
        status: 202,
      },
      {
        permission: "content.read",
        method: "GET",
        path: "/v1/workspaces/ws-1/projects/project-1/jobs/job-1",
        status: 200,
      },
      {
        permission: "content.read",
        method: "GET",
        path: "/v1/workspaces/ws-1/projects/project-1/assets/asset-1",
        status: 200,
      },
      {
        permission: "validation.read",
        method: "GET",
        path: "/v1/workspaces/ws-1/projects/project-1/validations",
        status: 200,
      },
      {
        permission: "approval.decide",
        method: "POST",
        path: "/v1/workspaces/ws-1/projects/project-1/approvals",
        headers: {
          "content-type": "application/json",
          "if-match": '"3"',
      ...
      },
      {
        ...
        method: "GET",
        path: "/v1/workspaces/ws-1/quota",
        status: 200,
      },
      {
        permission: "usage.read",
        method: "GET",
        path: "/v1/workspaces/ws-1/usage-records?page[size]=1",
        status: 200,
      },
      {
        permission: "audit.read",
        method: "GET",
        path: "/v1/workspaces/ws-1/audit-events?page[size]=1",
        status: 200,
      },
    ] as const;
    ...
    for (const item of cases) {
      ...
      expect(
        allowed.status,
        `${item.method} ${item.path} with ${item.permission}`
      ).toBe(item.status);
      ...
      const denied = await request({
        ...
        headers: {
          ...item.headers,
          "x-test-permission": "unrelated.permission",
        },
      ...
      expect(
        denied.status,
        `${item.method} ${item.path} without ${item.permission}`
      ).toBe(403);
      expect(JSON.parse(denied.body) as unknown).toMatchObject({
        code: "authorization_denied",
      });
  ...
  it("adapts shared OIDC authentication at the HTTP boundary", async () => {
    const authenticateOidc = createOidcRequestAuthenticator({
      authenticate: async (value: string | undefined) =>
        value === "Bearer valid" ? authenticate() : null,
    });
    await expect(
      authenticateOidc({
    ...
    await expect(
      authenticateOidc({ headers: {} } as http.IncomingMessage)
    ).resolves.toBeNull();
  ...
  it("maps API admission through the shared application handler", async () => {
    ...
    ).resolves.toMatchObject({ workflowRunId: "run-application" });
    await expect(
      admission(input, {
        workspaceId: "ws-1",
        projectId: "project-1",
    ...
    expect(calls).toEqual([
      {
        ...
        execution: expect.objectContaining({
          ...
          idempotency: {
            key: expect.stringMatching(/^v1:/u),
    ...
    const fingerprints = calls.map(
      (call) =>
        (
          call.execution as {
            readonly idempotency: { readonly fingerprint: string };
          }
