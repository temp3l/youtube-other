...
export async function createVeronicaPreImageReviewPack(
  ...
  const semanticReviews = z.object({ convergenceStatus: z.string().optional(), remediationRounds: z.number().int().nonnegative().optional(), providerReadiness: providerReadinessSchema.optional(), semanticQuality: semanticQualitySchema.optional(), reviews: z.array(z.object({ sceneId: z.string(), findings: z.array(z.object({ code: z.string(), severity: z.string(), message: z.string() })) })) }).parse(JSON.parse(semanticReviewRaw) as unknown);
  ...
  const canonicalConvergenceStatus = blockerCount === 0 && finalPlan.validation?.status !== "fail" && finalPlan.providerReadiness.status === "PASS"
    ? "CONVERGED"
    : "SEMANTIC_REMEDIATION_EXHAUSTED";
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(reviewManifestSchema.parse({
      ...
      semanticIntegrity: { status: blockerCount === 0 && finalPlan.validation?.status !== "fail" ? "PASS" : "FAIL", blockerCount: blockerCount + (finalPlan.validation?.status === "fail" ? 1 : 0), convergenceStatus: canonicalConvergenceStatus },
      ...
      overallPackValidity: integrity.status === "PASS" && blockerCount === 0 && finalPlan.validation?.status !== "fail" && finalPlan.providerReadiness.status === "PASS" && providerPromptQuality.status === "PASS" && semanticCoherenceIntegrity.status === "PASS" && finalPlan.semanticQuality.status === "PASS",
