...
export * from "./history-image-prompt.js";
...
function loadSceneStyleReference(): string {
  try {
    return readFileSync(
      path.join(repoRoot, "docs", "templates", "visual-scene-style.md"),
      "utf8"
    )
      .replace(/^##[^\n]*\n+/u, "")
      .trim();
...
export function createPromptBatch(
  scenePlan: ScenePlan,
  aspectRatio: "16:9" | "9:16",
  globalStyle: string,
  brandGuidance: string
  ...
  return scenePlan.scenes.map((scene) =>
    createImagePrompt(scene, aspectRatio, globalStyle, brandGuidance)
  );
}
...
function paperTexture(w: number, h: number, rng: () => number): string {
  ...
  for (let index = 0; index < dotCount; index += 1) {
    ...
    const r = 0.6 + rng() * 1.8;
    dots.push(
      `<circle cx="${x}" cy="${y}" r="${r}" fill="rgba(58,49,40,0.06)" />`
    );
  }
  return dots.join("");
}
...
function tornEdge(w: number, h: number, rng: () => number): string {
  const fiber = (x1: number, y1: number, x2: number, y2: number): string =>
    roughLine(x1, y1, x2, y2, "rgba(48,40,32,0.12)", 2, rng);
...
function mouseIcon(
  ...
  return [
    ...
    whisker3,
    ...paws,
    scribbleShadow(cx, cy + 26 * scale, scale * 0.9, rng),
  ].join("");
}
...
function childIcon(
  ...
  return [
    scribbleShadow(cx, cy + 30 * scale, scale * 0.8, rng),
    dress,
    head,
    headSketch,
    ...arms,
...
function birdIcon(
  ...
  return [
    scribbleShadow(cx, cy + 18 * scale, scale * 0.8, rng),
    body,
    bodySketch,
    wing,
    beak,
...
function cageIcon(
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    bars.push(
      roughLine(bx, y, bx + (rng() - 0.5) * 5, y + height, "#302820", 4, rng)
    );
  }
  return [
...
function nodesAndLinks(
  ...
  return [
    ...
    ...nodes.map((node) =>
      circle(node.x, node.y, 9 * scale, "#c86937", "#302820", 4)
    ),
  ].join("");
}
...
function compositionForScene(
  scene: Scene,
  width: number,
  height: number
): string {
  const text =
    `${scene.subject} ${scene.action} ${scene.setting} ${scene.visualPurpose} ${scene.canonicalNarration}`.toLowerCase();
  ...
  const props: string[] = [];
  props.push(
    paperCutout(w * 0.13, h * 0.16, w * 0.18, h * 0.12, "#f2d2b9", rng)
  );
  props.push(
    paperCutout(w * 0.66, h * 0.14, w * 0.19, h * 0.11, "#fff5d9", rng)
  );
  props.push(
    paperCutout(w * 0.1, h * 0.72, w * 0.24, h * 0.11, "#4a7f73", rng)
  );
  props.push(tornEdge(w, h, rng));
  if (has(/mouse|mice|colony|universe|experiment/)) {
    props.push(mouseIcon(w * 0.3, h * 0.58, Math.min(w, h) / 470, rng));
    ...
    props.push(
      personIcon(w * 0.17, h * 0.6, Math.min(w, h) / 500, rng, "#4a7f73")
    );
    props.push(
      personIcon(w * 0.78, h * 0.58, Math.min(w, h) / 530, rng, "#c86937")
    );
  ...
  if (has(/food|bowl|feed|abundant|resources/)) {
    props.push(bowlIcon(w * 0.7, h * 0.66, Math.min(w, h) / 540, "#4a7f73"));
  }
  if (has(/cage|enclosure|safe|roomy|space|laboratory|lab/)) {
    props.push(cageIcon(w * 0.7, h * 0.42, w * 0.3, h * 0.24, rng));
  }
  if (
    has(
      /crowd|population|climbing|aggressive|fight|stress|break|collapse|warning|effect|unravel/
    )
  ) {
    props.push(crackIcon(w * 0.2, h * 0.28, w * 0.8, h * 0.72, rng));
  ...
  if (has(/social|connection|roles|community|structure|belong|trust/)) {
    ...
    props.push(
      personIcon(w * 0.32, h * 0.58, Math.min(w, h) / 580, rng, "#c86937")
    );
    props.push(
      personIcon(w * 0.46, h * 0.59, Math.min(w, h) / 600, rng, "#4a7f73")
    );
    props.push(childIcon(w * 0.7, h * 0.61, Math.min(w, h) / 580, rng));
    props.push(birdIcon(w * 0.2, h * 0.31, Math.min(w, h) / 720, rng));
  }
  if (has(/healthy|groom|clean|beautiful ones|surviv/i)) {
    props.push(
      circle(w * 0.76, h * 0.3, Math.min(w, h) / 14, "#fff5d9", "#302820", 5)
    );
    props.push(
      roughLine(w * 0.76, h * 0.22, w * 0.76, h * 0.38, "#302820", 4, rng)
    );
    props.push(
      personIcon(w * 0.3, h * 0.62, Math.min(w, h) / 620, rng, "#fff5d9")
    );
  ...
  if (has(/lab|experiment|calhoun|universe|science/)) {
    ...
    props.push(
      personIcon(w * 0.24, h * 0.68, Math.min(w, h) / 700, rng, "#c86937")
    );
    props.push(mouseIcon(w * 0.7, h * 0.7, Math.min(w, h) / 760, rng));
  }
  ...
  props.push(
    personIcon(w * 0.52, h * 0.77, Math.min(w, h) / 720, rng, "#4a7f73")
  );
  return props.join("");
}
...
export async function createPlaceholderImage(
  ...
  <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
    ...
    ${paperCutout(width * 0.08, height * 0.1, width * 0.28, height * 0.18, "#f2d2b9", rng)}
    ${paperCutout(width * 0.64, height * 0.12, width * 0.22, height * 0.16, "#fff5d9", rng)}
    ${paperCutout(width * 0.16, height * 0.74, width * 0.2, height * 0.12, "#4a7f73", rng)}
...
export async function importImageAssets(
  ...
  for (const entry of files) {
    ...
    const expectedScene = scenePlan.scenes.find(
      (scene) =>
        scene.expectedImageFilenames.includes(entry.name) ||
        mapping[entry.name] === scene.id
    );
    if (!expectedScene) {
      continue;
    }
...
export function validateImageAssets(
  scenePlan: ScenePlan,
  assets: ReadonlyArray<ImageAsset>
): { readonly valid: boolean; readonly issues: string[] } {
  const issues: string[] = [];
  const seenChecksums = new Set<string>();
  const expected = new Set(
    scenePlan.scenes.flatMap((scene) => scene.expectedImageFilenames)
  );
...
export function buildSceneWorkbookSummary(
  prompts: ReadonlyArray<ImagePrompt>
): string {
  return prompts
    .map(
