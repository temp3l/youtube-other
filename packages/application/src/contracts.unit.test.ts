...
describe("application contracts", () => {
  ...
  it("accepts only active tenant-scoped pilot keys", () => {
    const key = "mfk_test-secret-with-enough-entropy";
    const firstHash = hashPilotApiKey(key);
    expect(hashPilotApiKey(key)).not.toBe(firstHash);
    ...
    expect(
      authenticatePilotApiKey({
        ...
        records: [
          {
            id: "key-1",
            secretHash: hashPilotApiKey(key),
            workspaceId: "workspace-1",
            permissions: [],
            expiresAt: "2026-08-01T00:00:00Z",
          },
  ...
  it("fails closed on budget caps and retains immutable audit facts", () => {
    ...
    ledger.reserve(
      { id: "reserve-1", workspaceId: "workspace-1", amount: 3 },
      5
    );
    expect(() =>
      ledger.reserve(
        { id: "reserve-2", workspaceId: "workspace-1", amount: 3 },
        5
      )
    ).toThrow(/quota/u);
    expect(ledger.audit("workspace-1")).toEqual([
      expect.objectContaining({ action: "workflow.admitted" }),
    ]);
  });

  it("requires reconciliation instead of retrying an uncertain publication", () => {
    const executing = transitionPublication(
      {
        id: "publication-1",
        approvalRevision: 1,
        credentialVersion: "credential-1",
        assetHash: "a".repeat(64),
        recoveryIdentity: "intent-1",
        state: "pending",
      },
      "executing"
    );
    ...
    expect(() => transitionPublication(uncertain, "executing")).toThrow(
      /transition/u
    );
    expect(transitionPublication(uncertain, "published")).toMatchObject({
      state: "published",
    });
  });
  ...
  it("binds exactly one publication receipt and never uploads during reconciliation", async () => {
    ...
    const worker = new PublicationReconciliationWorker(
      {
        findByRecoveryIdentity: async () => [
          {
            providerObjectId: "video-1",
            recoveryIdentity: "intent-1",
            evidence: { marker: "intent-1" },
          },
        ],
      },
      {
        ...
        recordInconclusive: async (input) => {
          writes.push(input);
        },
      }
    );
  ...
  it("keeps zero or multiple provider matches in operator reconciliation", async () => {
    ...
    ).resolves.toEqual({ kind: "reconciliation_required", reason: "no_match" });
    expect(reasons).toEqual(["no_match"]);
  });

  it("rejects a provider receipt for a different recovery identity", async () => {
    const reasons: string[] = [];
    const worker = new PublicationReconciliationWorker(
      {
        findByRecoveryIdentity: async () => [
          {
            ...
            recoveryIdentity: "intent-foreign",
            evidence: {},
          },
        ],
      },
      {
        recordResolved: async () => {
          throw new Error("mismatched receipt must not resolve");
        },
        recordInconclusive: async (input) => {
          reasons.push(input.reason);
        },
    ...
    expect(reasons).toEqual(["recovery_identity_mismatch"]);
  });

  it("keeps release scope internal until every pilot evidence cell passes", () => {
    expect(
      assessPilotGate({
        educationProviderFree: true,
        controlledProviderSmoke: false,
        tenantIsolation: true,
        objectStorage: true,
        webhooks: true,
        quotasAndAudit: true,
        publicationReconciliation: true,
        operationalRunbooks: true,
      })
    ).toEqual({ eligible: false, missing: ["controlledProviderSmoke"] });
  });
  ...
  it("delivers durable outbox events with an immutable consumer deduplication ID", async () => {
    ...
    const worker = new DurableOutboxWorker(
      {
        ...
        markOutboxDelivered: async (input) => {
          calls.push(`ack:${input.outboxId}:${input.leaseFence}`);
          return true;
        },
      ...
      },
      {
        dispatch: async (event) => {
          calls.push(`send:${event.id}:${event.attempt}`);
        },
      },
      {
        workerId: "worker-1",
    ...
    await expect(worker.dispatchOne("workspace-1")).resolves.toEqual({
      kind: "delivered",
      outboxId: "event-1",
    });
  ...
  it("uses one authorized, idempotent handler for connected workflow admission", async () => {
    ...
    const handler = new WorkflowAdmissionHandler({
      admit: async (input) => {
        admitted.push(input);
