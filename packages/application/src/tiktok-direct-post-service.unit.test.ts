...
import {
  TikTokDirectPostApplicationService,
  type TikTokDirectPostApplicationPort,
...
function buildApplicationPort(): TikTokDirectPostApplicationPort {
  const persistence = new InMemoryTikTokDirectPostPersistence();
  const adapter = new FixtureTikTokDirectPostAdapter("tiktok.publish.app");
  ...
  return {
    ...
    dispatchInit: (input) => service.dispatchInit(input),
  };
}
...
function buildDispatchInput() {
  ...
  const consent = {
    schemaVersion: "mediaforge.microdrama-publication.v1" as const,
    consentRevisionId: "consent.rev.app",
  ...
  const exportApproval = {
    schemaVersion: "mediaforge.microdrama-publication.v1" as const,
    exportApprovalRevisionId: "export.approval.app",
    ...
    metadataRevisionId: "meta.rev.app",
  ...
  const binding = {
    ...
    episodeId: "episode.app",
    episodeRevisionId: "rev.episode.app",
    locale: "en-US",
    renderHash,
    metadataRevisionId: "meta.rev.app",
    consentRevisionId: consent.consentRevisionId,
  ...
  const intent = planPublicationIntent({
    intentId: "intent.direct-post.app",
    targetProfile,
    binding,
    dispatchMode: "manual",
    idempotencyKey: "idempotency.direct-post.app",
  ...
  const attempt = preparePublicationAttempt({
    attemptId: "attempt.direct-post.app",
  ...
  const transferPlan = buildTikTokFileUploadTransferPlan({
    source: {
      relativePath: "renders/en-US/app-final.mp4",
      mimeType: "video/mp4",
      byteLength: 725,
  ...
  const creatorPreflight = {
    ...
    resolution: {
      ...
      targetProfile,
      accountFence: "fence.account.app",
      resolvedAt: evaluatedAt,
    },
    creatorInfo: {
      schemaVersion: "mediaforge.tiktok-creator-preflight.v1" as const,
      providerAccountId: targetProfile.providerAccountId,
      creatorOpenId: "tiktok.open-id.app",
      displayName: "Creator App",
  ...
  const configuration = planTikTokAppDeveloperConfigurationRevision({
    configurationRevisionId: "tiktok.app.config.app",
  ...
  const auditEvidence = recordTikTokProviderAuditEvidence({
    auditEvidenceId: "tiktok.app.audit.app",
    ...
    submissionReference: "provider.submission.app",
  ...
  const auditReadiness = projectTikTokAppAuditReadiness({
    readinessProjectionId: "tiktok.app.readiness.app",
    configuration,
    auditEvidence,
    recordedAt: evaluatedAt,
  });
  ...
  return {
    correlationId: "corr.direct-post.app",
    evaluatedAt,
    ...
    effectId: "effect.direct-post.app",
    initRequestId: "init.request.app",
...
describe("TikTok Direct Post application service", () => {
  it("delegates provider-free fake adapter dispatch through the application port", async () => {
    const service = new TikTokDirectPostApplicationService({
      port: buildApplicationPort(),
    });
    const input = buildDispatchInput();

    const admission = service.evaluateDispatchAdmission(input);
    expect(admission.allowed).toBe(true);

    const result = await service.dispatchInit(input);
    expect(result.effect?.state).toBe("init_dispatched");
    expect(result.effect?.initResponse?.publishId).toMatch(/^tiktok\.publish\.app\./);
  });
});
