...
export function extractCandidateGapInventoryV36(input: {
  ...
  for (const run of input.runs) {
    ...
    const atomicCandidateClaimIds = new Set(run.native.candidates
      .filter((candidate) => ["atomic-claim-grounding", "atomic-process-projection", "atomic-temporal-projection", "atomic-transforms-causal-projection", "atomic-evidence-set-projection", "atomic-approved-modal-causal-projection", "proof-aware-relation-evidence"].includes(candidate.source))
