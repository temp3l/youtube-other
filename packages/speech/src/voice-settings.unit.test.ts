...
function approval(overrides: Partial<ApprovalRecord> = {}): ApprovalRecord {
  return approvalRecordSchema.parse({
    ...
    id: "approval-voice-one",
    workflowInstanceId: "instance-001",
    taskId: "strategic.voice",
    ...
    reason: "Approved voice dispatch.",
...
function wavResponse(): Response {
  ...
  for (let index = 0; index < samples; index += 1) {
    bytes.writeInt16LE(
      Math.round(Math.sin(index / 31) * 8_000),
      44 + index * 2
    );
  }
...
describe("speech voice settings", () => {
  ...
  it("adapts instructions for the requested language", () => {
    ...
    expect(settings.narrationPacingPreset?.id).toBe(
      "dark-truth-es-full-pace-v1"
    );
  ...
  it("loads artifact-specific voice templates from config", () => {
    ...
    expect(fullTemplate.path).toBe(
      resolveSpeechVoiceInstructionPath("de", "full")
    );
    expect(fullTemplate.instructions).toContain("düsteren Dokumentarton");
    expect(shortTemplate.path).toBe(
      resolveSpeechVoiceInstructionPath("de", "short")
    );
  ...
  it("loads Italian pacing and pronunciation instructions from the locale template", () => {
    ...
    expect(settings.narrationPacingPreset?.id).toBe(
      "strategic-reinvention-it-full-pace-v1"
    );
    expect(resolveSpeechVoiceInstructionPath("it", "full")).toContain(
      "voices/strategic-reinvention/it-v1.txt"
    );
  ...
  it("blocks a missing dispatch context before any speech provider call", async () => {
    ...
    ).rejects.toThrow("explicit dispatch context");
      ...
      provider.synthesize(
        {
          ...
          text: "Blocked.",
          voiceProfile: loadSpeechVoiceSettings().profile,
          outputPath: "/tmp/strategic-legacy-voice.wav",
          dispatchContext: { kind: "legacy-noncreator" },
        },
        new AbortController().signal
      )
    ).rejects.toThrow("cannot dispatch speech as legacy noncreator");
    expect(create).not.toHaveBeenCalled();
  });
  ...
  it("fails every malformed or non-current creator approval matrix entry before provider dispatch", async () => {
    const cases: Array<[string, unknown]> = [
      ...
      [
        "wrong workflow",
        creatorContext([
          approval({ workflowInstanceId: "instance-002" as never }),
        ]),
      ],
      [
        "wrong task",
        creatorContext([approval({ taskId: "strategic.other" as never })]),
      ],
      [
        "wrong unit",
        creatorContext([approval({ unitId: "episode-002" as never })]),
      ],
      [
        "wrong profile",
        creatorContext([approval({ profileId: "dark-truth" })]),
      ],
      [
        "wrong gate",
        creatorContext([
          approval({ scope: { ...approval().scope!, gate: "metadata" } }),
        ]),
      ],
      [
        "wrong locale",
        creatorContext([
          approval({
            locale: "en",
            scope: { ...approval().scope!, locale: "en" },
          }),
        ]),
      ],
      [
        ...
        creatorContext([
          approval({
            variant: "short",
            scope: { ...approval().scope!, variant: "short" },
          }),
        ]),
      ],
      [
        ...
        creatorContext([
          approval({
            scope: {
              ...approval().scope!,
              inputArtifactHashes: ["c".repeat(64)],
            },
          }),
        ]),
      ],
      [
        ...
        creatorContext([
          approval({
            ...
            scope: {
              ...approval().scope!,
              outputArtifactHashes: ["c".repeat(64)],
            },
          }),
        ]),
      ],
      [
        "wrong revision",
        creatorContext([approval({ boundRevision: "revision-2" })]),
      ],
      [
        "expired",
        creatorContext([approval({ expiresAt: "2026-01-01T00:00:00.000Z" })]),
      ],
      [
        "later rejected",
        creatorContext([
      ...
      [
        ...
        creatorContext([
          approval(),
          approval({
            id: "approval-voice-revoked",
            decision: "revoked",
            actor: "reviewer@example.invalid",
            supersedesApprovalId: "approval-voice-one",
          }),
        ]),
      ],
      [
        "one high-risk actor",
        creatorContext([
          approval({ scope: { ...approval().scope!, highRisk: true } }),
        ]),
      ],
    ];
        ...
        provider.synthesize(
          {
            ...
            text: "Blocked.",
            voiceProfile: loadSpeechVoiceSettings().profile,
            outputPath: `/tmp/voice-policy-${name}.wav`,
            dispatchContext: dispatchContext as never,
          },
          new AbortController().signal
        )
      ).rejects.toThrow();
  ...
  it("blocks valid creator approvals because strategic synthetic narration is disabled", async () => {
    for (const approvals of [
      [
        approval(),
        approval({
          id: "approval-voice-two",
          actor: "two@example.invalid",
        ...
        provider.synthesize(
          {
            ...
            text: "Approved.",
            voiceProfile: loadSpeechVoiceSettings().profile,
            outputPath: "/tmp/voice-policy-approved.wav",
            dispatchContext: creatorContext(approvals),
          },
          new AbortController().signal
        )
      ).rejects.toThrow("Synthetic narration is disabled");
      expect(create).not.toHaveBeenCalled();
    }
  });
});
