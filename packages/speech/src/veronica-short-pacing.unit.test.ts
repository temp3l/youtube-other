...
describe("Veronica Short natural pacing", () => {
  ...
  it("uses the centralized locale-aware conceptual-explainer profile", () => {
    ...
    expect(
      resolveVeronicaShortPacingPolicy("en-US")?.platformMaximumDurationSeconds
    ).toBe(180);
  });

  it("targets natural WPM rather than 60 or 120 seconds", () => {
    const correction = estimateVeronicaShortPacingSpeed({
      currentSpeed: 1,
    ...
    ).toBe("LONG_SHORT_EDITORIAL_REVIEW");
  });

  it("keeps German provider-speed remediation within one bounded adjustment", () => {
    const germanPolicy = resolveVeronicaShortPacingPolicy("de")!;
    const correction = estimateVeronicaShortPacingSpeed({
      currentSpeed: 1.6,
      actualDurationSeconds: 60,
      wordCount: 210,
