...
export const taskApprovalRequirementSchema = z
  .object({
    gate: approvalGateSchema,
    highRisk: z.boolean().default(false),
    requiredDistinctActors: z.number().int().min(1).max(10).default(1),
  })
  .strict()
  .transform((value) => ({
    ...value,
    requiredDistinctActors: value.highRisk
      ? Math.max(2, value.requiredDistinctActors)
      : value.requiredDistinctActors,
  }));
export type TaskApprovalRequirement = z.infer<
  typeof taskApprovalRequirementSchema
>;
...
const taskPolicySchema = z
  .object({
    ...
    approval: taskApprovalRequirementSchema.optional(),
    batchable: z.boolean(),
...
export const taskDefinitionSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.policies.approval && !value.policies.approvalRequired) {
      ctx.addIssue({
        code: "custom",
        path: ["policies", "approvalRequired"],
        message: "A scoped approval policy requires approvalRequired.",
      });
    }
  });
...
export const approvalScopeSchema = z
  .object({
    ...
    outputArtifactHashes: z.array(sha256Schema).min(1),
    highRisk: z.boolean().default(false),
  })
  .strict();
...
export const approvalRecordSchema = z
  .object({
    ...
    // These fields extend the durable v1 record. They remain optional so that
    // existing v1 approval files are readable, but all scoped approvals must
    // include the complete scope and fingerprint binding below.
    scope: approvalScopeSchema.optional(),
    reviewRole: nonEmptyStringSchema.optional(),
    supersedesApprovalId: approvalIdSchema.optional(),
  })
  .strict()
  .superRefine((value, ctx) => {
    if (!value.scope) return;
    if (value.locale !== value.scope.locale || value.variant !== value.scope.variant) {
      ctx.addIssue({
        code: "custom",
        path: ["scope"],
        message: "Approval scope locale and variant must match the approval record.",
      });
    }
    if (!sameHashSet(value.artifactHashes, value.scope.outputArtifactHashes)) {
      ctx.addIssue({
        code: "custom",
        path: ["artifactHashes"],
        message: "Scoped approval output hashes must match artifact hashes.",
      });
    }
    if (value.decision === "revoked" && !value.supersedesApprovalId) {
      ctx.addIssue({
        code: "custom",
        path: ["supersedesApprovalId"],
        message: "A revoked approval must identify the approval it revokes.",
      });
    }
  });
...
export const workflowEventSchema = z.discriminatedUnion("eventType", [
  ...
  z
    .object({
      ...
      taskId: taskIdSchema,
      decision: z.enum(["approved", "rejected", "revoked"]).optional(),
