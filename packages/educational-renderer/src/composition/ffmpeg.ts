...
import { runProcess, type ProcessOptions } from "../infrastructure/process.js";
...
export async function encodeStaticScene(svgPath: string, outputPath: string, durationMs: number, profile: NormalizedRenderProfile, options: ProcessOptions = {}): Promise<number> { const result = await runProcess("ffmpeg", buildStaticSceneArgs(svgPath, outputPath, durationMs, profile), options); return result.durationMs; }
export async function composeSegments(args: { scenePaths: readonly string[]; concatPath: string; outputPath: string; durationMs: number; profile: NormalizedRenderProfile; audio?: { path: string; volume: number }; subtitles?: { path: string; mode: "embedded" | "none" }; signal?: AbortSignal }): Promise<number> {
  ...
  const result = await runProcess("ffmpeg", ffmpegArgs, { timeoutMs: 900_000, ...(args.signal ? { signal: args.signal } : {}) }); return result.durationMs;
}
