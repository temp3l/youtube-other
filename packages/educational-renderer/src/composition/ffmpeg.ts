...
export async function composeSceneSegments(args: { scenePaths: readonly string[]; concatPath: string; outputPath: string; durationMs: number; profile: NormalizedRenderProfile; signal?: AbortSignal }): Promise<ProcessResult> {
  const ffmpegArgs: string[] = ["-hide_banner", "-loglevel", "error", "-y", "-f", "concat", "-safe", "0", "-i", args.concatPath, "-an", "-c:v", args.profile.encoder, ...(args.profile.encoder === "libx264" ? ["-preset", args.profile.preset, "-threads", "1"] : []), "-pix_fmt", args.profile.pixelFormat, "-r", String(args.profile.frameRate), "-vf", `scale=${args.profile.width}:${args.profile.height}:flags=lanczos,format=${args.profile.pixelFormat}`, "-map_metadata", "-1", "-movflags", "+faststart", "-t", (args.durationMs/1000).toFixed(3), args.outputPath];
