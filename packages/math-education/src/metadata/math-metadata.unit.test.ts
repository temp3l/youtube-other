...
describe("production math metadata", () => {
  ...
  it.each(MATH_LANGUAGES)(
    "binds complete localized %s metadata to evidence and timing",
    async (language) => {
      ...
      expect(metadata.chapters.map((chapter) => chapter.beat)).toEqual([
        "opening",
        "example",
        "challenge",
        "solution",
      ...
      expect(
        metadata.chapters.every(
          (chapter) => chapter.seconds < input.timingEvidence.durationSeconds
        )
      ).toBe(true);
      ...
      if (language === "de")
        expect(input.evidence.localizedTopic).toBe("Stellenwert und Ziffer");
      if (language !== "de")
        expect(JSON.stringify(metadata)).not.toMatch(
          /\b(?:und|Klasse|Beispiel|Denkaufgabe|Mathematik)\b/u
        );
    }
  );
  ...
  it("uses only the complete release-derived order and real boundaries", async () => {
    ...
    const index = input.reviewedContext.stableTopologicalOrder.indexOf(
      input.skill.skillId
    );
    expect(metadata.dagNeighbors).toEqual({
      previousSkillId:
        input.reviewedContext.stableTopologicalOrder[index - 1] ?? null,
      nextSkillId:
        input.reviewedContext.stableTopologicalOrder[index + 1] ?? null,
    ...
    expect(() =>
      generateMathMetadata({
        ...
        reviewedContext: {
          ...input.reviewedContext,
          stableTopologicalOrderHash: "0".repeat(64),
        },
      })
    ).toThrow();
  });
  ...
  it("fails closed on identity, objective, timing, unsupported catalog, and unknown fields", async () => {
    ...
    expect(() =>
      generateMathMetadata({
        ...input,
        evidence: { ...input.evidence, lessonId: "wrong" },
      })
    ).toThrow(/identity/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        evidence: { ...input.evidence, objectiveHash: "0".repeat(64) },
      })
    ).toThrow(/identity|objective/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        timingEvidence: { ...input.timingEvidence, lessonId: "foreign" },
      })
    ).toThrow(/identity/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        timingEvidence: { ...input.timingEvidence, language: "de" },
      })
    ).toThrow(/identity/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        lesson: { ...input.lesson, contentHash: "0".repeat(64) },
      })
    ).toThrow(/identity|stale/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        reviewedContext: {
          ...input.reviewedContext,
          targetSkillHash: "0".repeat(64),
        },
      })
    ).toThrow(/identity|authoritative/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        workflowEvidence: {
    ...
    expect(() =>
      generateMathMetadata({
        ...
        catalog: {
          ...mathPlaylistCatalog,
          entries: mathPlaylistCatalog.entries.filter(
            (entry) => entry.key !== "topic-zo"
          ),
        },
      })
    ).toThrow(/Missing required playlist key|every supported key/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        catalog: {
    ...
    expect(() =>
      generateMathMetadata({
        ...
        catalog: {
          ...mathPlaylistCatalog,
          entries: [
            ...mathPlaylistCatalog.entries,
            mathPlaylistCatalog.entries[0]!,
          ],
        },
      })
    ).toThrow(/Duplicate playlist/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        catalog: {
          ...
          entries: [
            ...mathPlaylistCatalog.entries,
            { ...mathPlaylistCatalog.entries[0]!, key: "unknown-key" as never },
          ],
        },
      })
    ).toThrow();
    expect(() =>
      generateMathMetadata({
        ...
        catalog: {
          ...mathPlaylistCatalog,
          entries: mathPlaylistCatalog.entries.map((entry) =>
            entry.key === "topic-zo"
              ? { ...entry, kind: "grade" as const }
              : entry
          ),
        },
      })
    ).toThrow(/wrong kind/u);
    expect(() =>
      generateMathMetadata({
        ...input,
        evidence: { ...input.evidence, unsupported: true } as never,
      })
    ).toThrow();
  });
});
