...
import { metadataTimingEvidenceSchema, type MetadataTimingEvidence } from "../lesson/timing.js";
import { analyzePrerequisiteDag, prerequisitesFileSchema } from "../curriculum/dag.js";
...
const playlistKeySchema = z.enum([
  ...
  "topic-zo", "topic-gm", "topic-dz",
  "variant-foundation", "variant-standard", "variant-challenge",
]);
  ...
  .superRefine((catalog, context) => {
    ...
    if (catalog.entries.length !== playlistKeySchema.options.length)
      context.addIssue({
        code: "custom",
        path: ["entries"],
        message: "Playlist catalog must contain every supported key exactly once.",
      });
  });
...
export const mathPlaylistCatalog: MathPlaylistCatalog =
  mathPlaylistCatalogSchema.parse({
    ...
    entries: [
      ...
      ...(["foundation", "standard", "challenge"] as const).map(
        (variant) => ({
          key: `variant-${variant}`,
          kind: "variant",
          localizedNames: {
...
export const metadataWorkflowEvidenceSchema = z.strictObject({
  ...
  sources: z.strictObject({
    ...
    localization: z.strictObject({
      relativePath: z.string().regex(/^locales\/(?:de|en|es|fr|pt)\/narration\.json$/u),
    ...
    timing: z.strictObject({
      relativePath: z.string().regex(/^locales\/(?:de|en|es|fr|pt)\/timing\.json$/u),
...
export type MetadataWorkflowEvidence = z.infer<typeof metadataWorkflowEvidenceSchema>;
...
function createMetadataContext(release: {
  ...
  const recomputed = analyzePrerequisiteDag(release.skills, prerequisites.edges).order;
...
const localeCopy = {
  de: { region: "DE", opening: "Heute untersuchen wir", conjunction: "und", description: "Geprüftes Beispiel, Denkaufgabe und vollständige Lösung.", chapters: ["Start", "Beispiel", "Denkaufgabe", "Lösung"], math: "Mathematik", grade: "Klasse" },
  en: { region: "US", opening: "Today we investigate", conjunction: "and", description: "Verified example, challenge, and complete solution.", chapters: ["Start", "Example", "Challenge", "Solution"], math: "Mathematics", grade: "Grade" },
  es: { region: "419", opening: "Hoy investigamos", conjunction: "y", description: "Ejemplo verificado, reto y solución completa.", chapters: ["Inicio", "Ejemplo", "Reto", "Solución"], math: "Matemáticas", grade: "Grado" },
  fr: { region: "FR", opening: "Aujourd'hui, nous étudions", conjunction: "et", description: "Exemple vérifié, défi et solution complète.", chapters: ["Début", "Exemple", "Défi", "Solution"], math: "Mathématiques", grade: "Classe" },
  pt: { region: "BR", opening: "Hoje investigamos", conjunction: "e", description: "Exemplo verificado, desafio e solução completa.", chapters: ["Início", "Exemplo", "Desafio", "Solução"], math: "Matemática", grade: "Ano" },
} as const;
...
export function createMathMetadataEvidence(
  ...
  if (!opening?.startsWith(`${copy.opening} `))
    throw new Error("Localized topic evidence is missing from the opening beat.");
  const localizedTopic = opening
    .slice(copy.opening.length + 1)
    .split(".", 1)[0]
    ?.trim()
    .replace(/ und /gu, ` ${copy.conjunction} `);
  if (!localizedTopic || wordCount(localizedTopic) < 2 || wordCount(localizedTopic) > 5)
  ...
  if (!formulaFact) throw new Error("No localized verified fact evidence exists.");
...
function resolvePlaylist(
  ...
  if (entry.kind !== kind)
    throw new Error(`Playlist ${key} has wrong kind ${entry.kind}; expected ${kind}.`);
  return { key, kind, localizedName: entry.localizedNames[language] };
}

export function generateMathMetadata(input: GenerateMathMetadataInput): MathMetadata {
  if (
  ...
  const timingEvidence = metadataTimingEvidenceSchema.parse(input.timingEvidence);
  if (
  ...
  const catalog = mathPlaylistCatalogSchema.parse(input.catalog ?? mathPlaylistCatalog);
  ...
  const recomputedOrder = analyzePrerequisiteDag(context.skills, context.prerequisites.edges).order;
  const releaseSkill = context.skills.find((candidate) => candidate.skillId === skill.skillId);
  if (
    ...
    context.targetSkillId !== skill.skillId || !releaseSkill ||
    ...
    workflowEvidence.sources.localization.contentHash !== localization.contentHash ||
    workflowEvidence.sources.timing.contentHash !== canonicalHash(timing) ||
    context.stableTopologicalOrderHash !== canonicalHash(context.stableTopologicalOrder) ||
    canonicalHash(context.stableTopologicalOrder) !== canonicalHash(recomputedOrder) ||
    ...
    timingEvidence.lessonId !== lesson.lessonId || timingEvidence.skillId !== skill.skillId ||
    timingEvidence.variant !== lesson.variant || timingEvidence.language !== language ||
    ...
    canonicalHash(timingEvidence.orderedSceneIds) !== canonicalHash(timing.scenes.map((scene) => scene.sceneId)) ||
    canonicalHash(timingEvidence.orderedSegmentIds) !== canonicalHash(timing.scenes.map((scene) => scene.segmentId))
  )
    throw new Error("Math metadata identity or objective evidence mismatch.");
  if (!lesson.facts.some((fact) => fact.factId === evidence.verifiedFormulaFactId))
    throw new Error("Metadata formula fact is not present in the authoritative lesson.");
  if (wordCount(evidence.thumbnailText) < 2 || wordCount(evidence.thumbnailText) > 5)
  ...
  if (!context.rolloutCapability.variants.includes(lesson.variant))
    throw new Error("Metadata rollout capability does not include the lesson variant.");
  ...
  if (!topicCode) throw new Error(`Unsupported skill identity: ${skill.skillId}`);
  ...
  if (chapters.some((chapter, index) => chapter.seconds >= timing.durationSeconds || (index > 0 && chapter.seconds <= chapters[index - 1]!.seconds)))
    throw new Error("Metadata chapters must be monotone and within authoritative timing.");
  ...
  return mathMetadataSchema.parse({
    ...
    chapters,
    tags: [copy.math, `${copy.grade} ${skill.canonicalGrade}`, ...evidence.searchTerms],
