...
import {
  ...
  type StrategicReinventionContent,
} from "./index.js";
...
describe("Mediaforge API SDK", () => {
  ...
  it("maps bulk launch with a batch-scoped idempotency key", async () => {
    let request: { readonly url: string; readonly init?: RequestInit } | undefined;
    const client = new MediaforgeApiClient({ baseUrl: "https://api.example.test", request: (async (url: string | URL | Request, init?: RequestInit) => { request = { url: String(url), ...(init ? { init } : {}) }; return jsonResponse({ id: "batch-1", accepted: [], rejected: [] }, { status: 202 }); }) as typeof fetch });
    await client.launchBulkProduction("ws-1", "batch/one", { idempotencyKey: "bulk-launch-1" });
    expect(new URL(request!.url).pathname).toBe("/v1/workspaces/ws-1/bulk-production-batches/batch%2Fone:launch");
  ...
  it("types the Veronica Benini strategic source-reference contract", () => {
    const content: StrategicReinventionContent = {
    ...
    expect(content.sourceAssetIds).toEqual(["source-deck-1"]);
  });
  ...
  it("maps every current operation to its typed HTTP route and headers", async () => {
    ...
    const client = new MediaforgeApiClient({
      ...
      request: (async (url: string | URL | Request, init?: RequestInit) => {
        ...
        return jsonResponse(bodies.shift(), {
          ...
          headers: { "x-request-id": "request-server", etag: '"0"' },
        });
      }) as typeof fetch,
    });
    ...
    await client.createProject("ws-1", {
      name: "Project",
      profile: "dark_truth",
    });
    await client.createEpisode("ws-1", "project-1", {
      content: {
        type: "dark_truth",
        version: "1",
        premise: "p",
    ...
    await client.replaceEpisodeContent(
      "ws-1",
      "project-1",
      "episode-1",
      {
    ...
    await client.cancelWorkflow("ws-1", "project-1", "run-1", {
      ifMatch: '"0"',
    });
    await client.resumeWorkflow("ws-1", "project-1", "run-1", {
      ifMatch: '"1"',
      idempotencyKey: "resume-key",
    });
    await client.getJob("ws-1", "project-1", "job-1");
    await client.getAsset("ws-1", "project-1", "asset-1");
    await client.listValidations("ws-1", "project-1", {
      size: 25,
      after: "cursor",
    });
    await client.getPublication("ws-1", "project-1", "publication/one");
    await client.recordApproval(
      "ws-1",
      "project-1",
      {
      ...
      { ifMatch: '"1"', idempotencyKey: "approval-key" }
    );
    await client.revokeApproval(
      "ws-1",
      "project-1",
      "approval/one",
      { reason: "Superseded" },
      { ifMatch: '"1"', idempotencyKey: "approval-revoke-key" }
    );
    ...
    expect(requests.map((request) => new URL(request.url).pathname)).toEqual([
      ...
      "/health/ready",
      "/v1/openapi.json",
      "/v1/workspaces/ws-1/projects",
      "/v1/workspaces/ws-1/projects/project-1/episodes",
      "/v1/workspaces/ws-1/projects/project-1/episodes/episode-1",
      "/v1/workspaces/ws-1/projects/project-1/episodes/episode-1",
      "/v1/workspaces/ws-1/projects/project-1/episodes/episode-1/workflow-runs",
      "/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1",
      "/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1/steps",
      "/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1:cancel",
      "/v1/workspaces/ws-1/projects/project-1/workflow-runs/run-1:resume",
      "/v1/workspaces/ws-1/projects/project-1/jobs/job-1",
      "/v1/workspaces/ws-1/projects/project-1/assets/asset-1",
      "/v1/workspaces/ws-1/projects/project-1/validations",
      ...
      "/v1/workspaces/ws-1/quota",
      "/v1/workspaces/ws-1/usage-records",
      "/v1/workspaces/ws-1/audit-events",
    ]);
    expect(requests[6]!.init?.method).toBe("PATCH");
    expect(new Headers(requests[6]!.init?.headers).get("if-match")).toBe('"0"');
    expect(new URL(requests[14]!.url).searchParams.get("page[after]")).toBe(
      "cursor"
    );
    expect(new Headers(requests[7]!.init?.headers).get("idempotency-key")).toBe(
      "admit-key"
    );
    expect(new Headers(requests[10]!.init?.headers).get("if-match")).toBe(
      '"0"'
    );
    expect(
      new Headers(requests[11]!.init?.headers).get("idempotency-key")
    ).toBe("resume-key");
    expect(new Headers(requests[16]!.init?.headers).get("authorization")).toBe(
      "Bearer token"
    );
    expect(new Headers(requests[16]!.init?.headers).get("x-request-id")).toBe(
      "request-sdk"
    );
    expect(new Headers(requests[17]!.init?.headers).get("if-match")).toBe(
      '"1"'
    );
    expect(
      new Headers(requests[17]!.init?.headers).get("idempotency-key")
    ).toBe("approval-revoke-key");
    expect(new URL(requests[19]!.url).searchParams.get("page[after]")).toBe(
      "usage-after"
    );
    expect(new URL(requests[20]!.url).searchParams.get("page[size]")).toBe(
      "10"
    );
  });
  ...
  it("parses RFC 9457 problems and throws a typed API error", async () => {
    ...
    const error = await client
      .createProject("ws-1", { name: "Project", profile: "dark_truth" })
  ...
  it("polls jobs using Retry-After and honors abort signals", async () => {
    ...
    const client = new MediaforgeApiClient({
      ...
      request: (async () =>
        jsonResponse(jobs.shift(), {
          headers: { "retry-after": "2" },
        })) as typeof fetch,
    });
    await expect(
      client.pollJob("ws-1", "project-1", "job-1")
    ...
    await expect(
      client.pollJob("ws-1", "project-1", "job-1", { signal: aborted.signal })
  ...
  it("iterates validation cursors without exposing pagination mechanics", async () => {
    ...
    const ids: string[] = [];
    for await (const validation of client.iterateValidations(
      "ws-1",
      "project-1",
      { size: 1 }
    ))
  ...
  it("provides safe header, ETag, idempotency, and webhook verification helpers", () => {
    expect(withRequestId(undefined, "request-1").get("x-request-id")).toBe(
      "request-1"
    );
    expect(withIdempotencyKey(undefined, "key-1").get("idempotency-key")).toBe(
      "key-1"
    );
