...
function validateRequestId(value: string): void {
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{2,159}$/u.test(value))
    throw new TypeError(
      "requestId must be a safe identifier between 3 and 160 characters."
    );
}
...
function validateIdempotencyKey(value: string): void {
  if (value.length < 1 || value.length > 255 || !/^[\x20-\x7E]+$/u.test(value))
    throw new TypeError(
      "idempotencyKey must be printable ASCII and at most 255 characters."
    );
}
...
export type SdkHeadersInit =
  | Headers
  | Record<string, string>
  | [string, string][];

export function withRequestId(
  headers: SdkHeadersInit | undefined,
  requestId: string
): Headers {
...
export function createIdempotencyKey(prefix = "sdk"): string {
  if (!/^[A-Za-z0-9._-]+$/u.test(prefix))
    throw new TypeError("prefix must be a safe identifier.");
  return `${prefix}-${crypto.randomUUID()}`;
}

function encodePath(value: string): string {
  if (value.length === 0)
    throw new TypeError("Resource identifiers must not be empty.");
...
export class MediaforgeApiClient {
  ...
  private async headers(input: {
    ...
    let headers = new Headers(
      input.json ? { "content-type": "application/json" } : undefined
    );
    const token =
    ...
    if (input.idempotencyKey)
      headers = withIdempotencyKey(headers, input.idempotencyKey);
  ...
  private async execute<T>(
    ...
    if (!response.ok) {
      const problem = parseApiProblem(parsed) ?? {
        ...
        detail:
          "The API request failed without a valid Problem Details response.",
  ...
  public getOpenApiDocument(
    ...
    return this.execute("/v1/openapi.json", {
      ...(options ? { options } : {}),
    });
  }
  ...
  public getQuota(
    workspaceId: string,
    options?: RequestOptions
  ): Promise<ApiResponse<WorkspaceQuotaStatus>> {
    return this.execute(`${this.workspacePath(workspaceId)}/quota`, {
      ...(options ? { options } : {}),
    });
  }
  ...
  public listUsageRecords(
    ...
    return this.execute(
      `${this.workspacePath(workspaceId)}/usage-records${suffix}`,
      { options }
    );
  }

  public async *iterateUsageRecords(
    workspaceId: string,
    options: WorkspaceListOptions = {}
  ): AsyncGenerator<UsageRecord, void, void> {
    let after = options.after;
    do {
      const page = await this.listUsageRecords(workspaceId, {
        ...options,
        ...(after ? { after } : {}),
      });
  ...
  public listAuditEvents(
    workspaceId: string,
    options: WorkspaceListOptions = {}
  ): Promise<ApiResponse<AuditEventPage>> {
    const query = new URLSearchParams();
    if (options.size !== undefined)
      query.set("page[size]", String(options.size));
    if (options.after !== undefined) query.set("page[after]", options.after);
    const suffix = query.size > 0 ? `?${query.toString()}` : "";
    return this.execute(
      `${this.workspacePath(workspaceId)}/audit-events${suffix}`,
      { options }
    );
  }

  public async *iterateAuditEvents(
    workspaceId: string,
    options: WorkspaceListOptions = {}
  ): AsyncGenerator<AuditEvent, void, void> {
    let after = options.after;
    do {
      const page = await this.listAuditEvents(workspaceId, {
        ...options,
        ...(after ? { after } : {}),
      });
      for (const item of page.data.items) yield item;
  ...
  public createProject(
    ...
    return this.execute(`${this.workspacePath(workspaceId)}/projects`, {
      method: "POST",
      body: input,
      ...(options ? { options } : {}),
    });
  ...
  public createEpisode(
    ...
    input: EpisodeInput,
    options?: RequestOptions
  ): Promise<ApiResponse<EpisodeCreated>> {
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/episodes`,
  ...
  public replaceEpisodeContent(
    ...
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/episodes/${encodePath(episodeId)}`,
      { method: "PATCH", body: input, options, ifMatch: options.ifMatch }
    );
  }

  public admitWorkflow(
    workspaceId: string,
    projectId: string,
    episodeId: string,
    input: WorkflowAdmission,
    options: IdempotentRequestOptions
  ): Promise<ApiResponse<WorkflowCommandAccepted>> {
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/episodes/${encodePath(episodeId)}/workflow-runs`,
      {
        method: "POST",
        body: input,
        options,
  ...
  public getWorkflow(
    ...
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/workflow-runs/${encodePath(runId)}`,
      { ...(options ? { options } : {}) }
    );
  }
  ...
  public listWorkflowSteps(
    ...
    runId: string,
    options?: RequestOptions
  ): Promise<ApiResponse<WorkflowStepPage>> {
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/workflow-runs/${encodePath(runId)}/steps`,
      { ...(options ? { options } : {}) }
    );
  }
  ...
  public cancelWorkflow(
    ...
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/workflow-runs/${encodePath(runId)}:cancel`,
  ...
  public resumeWorkflow(
    ...
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/workflow-runs/${encodePath(runId)}:resume`,
      {
        method: "POST",
        options,
        ifMatch: options.ifMatch,
  ...
  public getJob(
    ...
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/jobs/${encodePath(jobId)}`,
      { ...(options ? { options } : {}) }
    );
  }
  ...
  public getAsset(
    ...
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/assets/${encodePath(assetId)}`,
      { ...(options ? { options } : {}) }
    );
  }
  ...
  public listValidations(
  ...
  ): Promise<ApiResponse<ValidationPage>> {
    const query = new URLSearchParams();
    if (options.size !== undefined)
      query.set("page[size]", String(options.size));
    if (options.after !== undefined) query.set("page[after]", options.after);
    const suffix = query.size > 0 ? `?${query.toString()}` : "";
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/validations${suffix}`,
      { options }
    );
  }
  ...
  public recordApproval(
    ...
    input: ApprovalInput,
    options: ConditionalIdempotentRequestOptions
  ): Promise<ApiResponse<ApprovalAccepted>> {
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/approvals`,
      {
        method: "POST",
        body: input,
        options,
  ...
  public revokeApproval(
    ...
    options: ConditionalIdempotentRequestOptions
  ): Promise<ApiResponse<ApprovalRevoked>> {
    return this.execute(
      `${this.projectPath(workspaceId, projectId)}/approvals/${encodePath(approvalId)}:revoke`,
      {
        method: "POST",
        body: input,
        options,
  ...
  public async pollJob(
    ...
    while (true) {
      ...
      const delay =
        retryAfterMilliseconds(result.headers.get("retry-after")) ??
        defaultInterval;
...
export function verifyWebhookSignature(input: {
  ...
  if (
    ...
    tolerance < 0 ||
    Math.abs(now.getTime() - timestamp.getTime()) > tolerance
  )
    return false;
