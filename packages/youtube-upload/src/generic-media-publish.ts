...
function canonicalJson(value: unknown): string {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value))
      throw new Error(
        "Publish fingerprint cannot contain a non-finite number."
      );
...
export const genericYoutubePublishReportSchema = z
  .strictObject({
  ...
  .superRefine((report, context) => {
    ...
    if (new Set(ids).size !== ids.length)
      context.addIssue({
        ...
        message: "Prior publish report contains duplicate playlist results.",
      });
    if (
      !report.videoId &&
      (report.thumbnailStatus === "assigned" ||
        report.playlistResults.some((entry) => entry.status === "assigned"))
    )
      context.addIssue({
        code: "custom",
        path: ["videoId"],
        message:
          "Prior publish report claims mutations without a video identity.",
      });
    const complete =
    ...
    if (
      ...
      (complete && report.blockers.length === 0)
    )
      context.addIssue({
        code: "custom",
        path: ["status"],
        message:
          "Prior publish report status contradicts its operation evidence.",
      });
    for (const [index, entry] of report.playlistResults.entries()) {
      if ((entry.status === "assigned") !== (entry.error === null))
        context.addIssue({
          code: "custom",
          path: ["playlistResults", index, "error"],
          message: "Playlist result status and error contradict.",
        });
      if (entry.status === "assigned" && entry.retryable !== null)
        context.addIssue({
          code: "custom",
          path: ["playlistResults", index, "retryable"],
          message: "Successful playlist evidence cannot be retryable.",
        });
      if (
        entry.bindingHash !==
        operationBinding({
        ...
        context.addIssue({
          code: "custom",
          path: ["playlistResults", index, "bindingHash"],
          message:
            "Playlist operation evidence is not bound to the request and video.",
        });
    }
    ...
    if (report.videoBindingHash !== expectedVideoBinding)
      context.addIssue({
        code: "custom",
        path: ["videoBindingHash"],
        message: "Video operation evidence is not bound to the request.",
      });
    const expectedThumbnailBinding =
      report.thumbnailStatus === "not-attempted"
        ? null
        : operationBinding({
            requestFingerprint: report.requestFingerprint,
            operation: "thumbnails.set",
            videoId: report.videoId,
            status: report.thumbnailStatus,
          });
    if (report.thumbnailBindingHash !== expectedThumbnailBinding)
      context.addIssue({
        code: "custom",
        path: ["thumbnailBindingHash"],
        message:
          "Thumbnail operation evidence is not bound to the request and video.",
      });
    const expectedVerificationBinding =
      report.verificationStatus === "not-attempted"
        ? null
        : operationBinding({
            requestFingerprint: report.requestFingerprint,
    ...
    if (report.verificationBindingHash !== expectedVerificationBinding)
      context.addIssue({
        code: "custom",
        path: ["verificationBindingHash"],
        message: "Verification evidence is not bound to the request and video.",
      });
    if (
      report.thumbnailStatus !== "failed" &&
      report.thumbnailRetryable !== null
    )
      context.addIssue({
        code: "custom",
        path: ["thumbnailRetryable"],
        message: "Only failed thumbnail operations have retry classification.",
      });
    if (
      report.verificationStatus !== "failed" &&
      report.verificationRetryable !== null
    )
      context.addIssue({
        code: "custom",
        path: ["verificationRetryable"],
        message:
          "Only failed verification operations have retry classification.",
      });
  });

export type YoutubeMediaClient = YoutubeMutationClient;
export type GenericYoutubePublishReport = z.infer<
  typeof genericYoutubePublishReportSchema
>;
...
function report(args: {
  ...
  playlistResults?: Array<
    Omit<GenericYoutubePublishReport["playlistResults"][number], "bindingHash">
  >;
  ...
  const value = genericYoutubePublishReportSchema.parse({
    ...
    thumbnailStatus,
    thumbnailRetryable:
      thumbnailStatus === "failed" ? (args.thumbnailRetryable ?? false) : null,
    thumbnailBindingHash:
      thumbnailStatus === "not-attempted"
        ? null
        : operationBinding({
            requestFingerprint: args.fingerprint,
            operation: "thumbnails.set",
    ...
    verificationRetryable:
      verificationStatus === "failed"
        ? (args.verificationRetryable ?? false)
        : null,
...
export async function saveGenericYoutubePublishReport(args: {
  ...
  if (reportAuthority.get(args.report) !== canonicalReportHash(args.report))
    throw new Error(
      "Only an unmodified publisher-owned report can be persisted."
    );
  const fileName = args.fileName ?? "youtube-media-publish.json";
  if (!/^[a-z0-9][a-z0-9.-]*\.json$/u.test(fileName))
    throw new Error("Invalid publish report name.");
...
export async function loadGenericYoutubePublishReport(args: {
  ...
  if (!target.startsWith(`${root}${path.sep}`))
    throw new Error("Publish report escapes its authoritative root.");
  const [rootReal, stat] = await Promise.all([
    fs.realpath(root),
    fs.lstat(target),
  ]);
  if (!stat.isFile() || stat.isSymbolicLink())
    throw new Error("Publish report is not a regular authoritative file.");
  const targetReal = await fs.realpath(target);
  if (!targetReal.startsWith(`${rootReal}${path.sep}`))
    throw new Error("Publish report symlink escapes its authoritative root.");
  if ((await hashFile(target)) !== args.expectedContentHash)
    throw new Error("Publish report content hash mismatch.");
  const value = genericYoutubePublishReportSchema.parse(
...
export async function publishYoutubeMedia(
  ...
  const validationBlockers = [
    ...
    ...(!policyResult.success
      ? policyResult.error.issues.map(
          (issue) => `Missing or invalid policy: ${issue.path.join(".")}.`
        )
      : []),
    ...(!metadataResult.success ? ["Invalid publish metadata."] : []),
    ...(!input.channelId.trim() ? ["Missing channel target."] : []),
    ...(playlistIds.length === 0 || playlistIds.some((id) => id.length === 0)
      ? ["Missing required playlist policy."]
      : []),
  ];
  if (validationBlockers.length > 0)
    return {
      report: report({
        ...
        identity,
        fingerprint,
        channelId: input.channelId || "invalid",
        videoHash,
        thumbnailHash,
        metadataHash,
  ...
  let currentEvidence: PublishDryRunEvidence;
  try {
    currentEvidence = await createPublishDryRunEvidence({
      identity: {
        contentId: identityResult.data.contentId,
        locale: identityResult.data.language,
        variant: identityResult.data.variant,
      },
      target: {
        channelId: input.channelId,
      ...
      artifacts: [
        {
          kind: "render",
          revision: input.artifactRevisions.media,
          path: input.mediaPath,
        },
        {
          kind: "thumbnail",
          revision: input.artifactRevisions.thumbnail,
          path: input.thumbnailPath,
        },
      ],
      metadata: {
        revision: input.metadataRevision,
    ...
    if (
      canonicalJson(currentEvidence) !== canonicalJson(input.dryRunEvidence)
    ) {
      throw new Error("Supplied publish dry-run evidence is stale.");
    }
    assertCurrentPublishApproval({
      ...
      report: report({
        ...
        blockers: [
          error instanceof Error
            ? error.message
            : "Current publish approval validation failed.",
        ],
      }),
      reused: false,
    };
  ...
  if (input.priorReport !== undefined) {
    const parsed = genericYoutubePublishReportSchema.safeParse(
      input.priorReport
    );
    if (
      !parsed.success ||
      ...
      return {
        report: report({
          ...
          metadataHash,
          blockers: [
            "Prior publish report is malformed or not authority-loaded.",
          ],
        }),
        reused: false,
      };
    ...
    if (
      ...
      prior.playlistResults.some(
        (entry, index) => entry.playlistId !== playlistIds[index]
      )
    )
      return {
        report: report({
          ...
          thumbnailHash,
          metadataHash,
          blockers: ["Prior publish report is stale or identity-mismatched."],
        }),
        reused: false,
      };
    if (
      prior.status === "PUBLISHED" &&
      playlistIds.every((id) =>
        prior!.playlistResults.some(
          (entry) => entry.playlistId === id && entry.status === "assigned"
        )
      )
    )
  ...
  const checkpointPlaylists = new Map(
    prior?.playlistResults.map((entry) => [
      entry.playlistId,
      {
        playlistId: entry.playlistId,
        status: entry.status,
  ...
  let checkpointThumbnailStatus: GenericYoutubePublishReport["thumbnailStatus"] =
    prior?.thumbnailStatus ?? "not-attempted";
  let checkpointVerificationStatus: GenericYoutubePublishReport["verificationStatus"] =
    prior?.verificationStatus ?? "not-attempted";
  ...
  const progress = await executeYoutubeMutationSequence({
    ...
    uploadRequest: {
      ...
      requestBody: {
        ...
        status: {
          privacyStatus: policy.privacyStatus,
          selfDeclaredMadeForKids: policy.madeForKids,
          containsSyntheticMedia: policy.containsSyntheticMedia,
        },
      },
    ...
    ...(prior
      ? {
          prior: {
            ...
            videoRetryAllowed:
              Boolean(prior.videoId) || prior.videoRetryable === true,
            thumbnailAssigned: prior.thumbnailStatus === "assigned",
            assignedPlaylistIds: prior.playlistResults
              .filter((entry) => entry.status === "assigned")
              .map((entry) => entry.playlistId),
            thumbnailRetryAllowed:
              prior.thumbnailStatus !== "failed" ||
              prior.thumbnailRetryable === true,
            nonRetryablePlaylistIds: prior.playlistResults
              .filter(
                (entry) => entry.status === "failed" && entry.retryable !== true
              )
              .map((entry) => entry.playlistId),
          },
        }
  ...
  if (
    ...
    !blockers.some((item) => item.includes("Thumbnail"))
  )
    blockers.push("Thumbnail assignment is incomplete.");
  if (
    verificationStatus !== "verified" &&
    !blockers.some((item) => item.includes("verification"))
  )
    blockers.push("Upload verification is incomplete.");
  const completed =
    blockers.length === 0 &&
