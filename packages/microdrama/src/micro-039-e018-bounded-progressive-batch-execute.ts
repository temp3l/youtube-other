...
import {
  ...
  isMicro039E018OutOfScopeEpisodeId,
  ...
  MICRO_039_E018_EPISODE_RANGE,
  MICRO_039_E018_PUBLICATION_ATTEMPT_ID,
  MICRO_039_E018_PUBLICATION_IDEMPOTENCY_KEY,
  MICRO_039_E018_PUBLICATION_INTENT_ID,
  MICRO_039_E018_TASK_ID,
  type Micro039E018BatchLocale,
...
import {
  buildMicro039E018ExplicitExecuteAuthorizationRecord,
  computeMicro039E018PreparationFingerprint,
  loadMicro039E018ExplicitExecuteAuthorization,
  persistMicro039E018ExplicitExecuteAuthorization,
  summarizeMicro039E018PreparationForFingerprint,
} from "./micro-039-e018-explicit-execute-authorization.js";
import { evaluateMicro039E018ProgressiveBatchPreflight } from "./micro-039-e018-progressive-batch-preflight.js";

export const MICRO_039_E018_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY =
  "microdrama.batch-execution-evidence.MICRO-039-E018";

export type Micro039E018ProgressiveBatchPorts = {
...
export function createMicro039E018ProgressiveBatchPorts(): Micro039E018ProgressiveBatchPorts {
  ...
  return {
    ...
    directPostAdapter: new FixtureTikTokDirectPostAdapter("tiktok.publish.micro-039-e018"),
    auditRepository,
  };
}

export type Micro039E018EpisodeBatchEvidence = {
  readonly locale: Micro039E018BatchLocale;
  readonly episodeId: "E018";
...
export type Micro039E018AuthorizeExplicitExecuteInput = {
...
export type Micro039E018AuthorizeExplicitExecuteResult = {
...
export type Micro039E018BoundedProgressiveBatchExecuteInput = {
  readonly dbPath: string;
  ...
  readonly ports: Micro039E018ProgressiveBatchPorts;
...
export type Micro039E018BoundedProgressiveBatchExecuteResult = {
  ...
  readonly episodes: readonly Micro039E018EpisodeBatchEvidence[];
...
export async function authorizeMicro039E018BoundedProgressiveBatchExplicitExecute(
  input: Micro039E018AuthorizeExplicitExecuteInput
): Promise<Micro039E018AuthorizeExplicitExecuteResult> {
  ...
  const { micro036Evidence, micro038Evidence, micro039E017Evidence, micro042Evidence } =
    loadUpstream({
    ...
    ...(input.micro038EvidenceJsonPath
      ...
      : {}),
    ...(input.micro039E017EvidenceJsonPath
      ? { micro039E017EvidenceJsonPath: input.micro039E017EvidenceJsonPath }
      : {}),
  ...
  const preflight = evaluateMicro039E018ProgressiveBatchPreflight({
  ...
  const bindingProbe = buildMicro039E018BatchBindingProbe({ revisionSet });
  const preparationSummary = summarizeMicro039E018PreparationForFingerprint(
    microdramaRepository,
    {
      micro036EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e018.micro-036",
        micro036Evidence
      ),
      micro038EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e018.micro-038",
        micro038Evidence
      ),
      micro039E017EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e018.micro-039-e017",
        micro039E017Evidence
      ),
      micro042EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e018.micro-042",
        micro042Evidence
      ),
  ...
  const preparationFingerprint = computeMicro039E018PreparationFingerprint(
    preparationSummary
  );

  const record = buildMicro039E018ExplicitExecuteAuthorizationRecord({
...
export async function executeMicro039E018BoundedProgressiveBatch(
  input: Micro039E018BoundedProgressiveBatchExecuteInput
): Promise<Micro039E018BoundedProgressiveBatchExecuteResult> {
  const blockers: string[] = [];
  const operatorId = input.operatorId ?? "operator.microdrama";
  const packRoot = input.packRoot ?? defaultV5PackRoot;
  ...
  const executeAuthorization =
    loadMicro039E018ExplicitExecuteAuthorization(microdramaRepository);
  const operatorAuthorization = loadMicro039E018OperatorAuthorization(microdramaRepository);
  const assetGenerationApproval =
    loadMicro039E018AssetGenerationApproval(microdramaRepository);
  const costBudgetApproval = loadMicro039E018CostBudgetApproval(microdramaRepository);
  ...
  for (const episodeId of input.requestedEpisodeIds ?? MICRO_039_E018_BATCH_EPISODE_IDS) {
    if (isMicro039E018OutOfScopeEpisodeId(episodeId)) {
  ...
  if (blockers.length > 0) {
    return {
      ...
      evidenceProjectionKey: MICRO_039_E018_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY,
    };
  }
  ...
  for (const profile of defaultMicro039BudgetProfiles({
    evaluatedAt: input.executedAt,
    episodeIds: MICRO_039_E018_BATCH_EPISODE_IDS,
  })) {
  ...
  const estimatedCostMinor = Math.floor(
    MICRO_039_E018_BATCH_COST_LIMIT_MINOR / MICRO_039_E018_BATCH_LOCALES.length
  );
  ...
  const episodes: Micro039E018EpisodeBatchEvidence[] = [];
  ...
  for (const episodeId of MICRO_039_E018_BATCH_EPISODE_IDS) {
    for (const locale of MICRO_039_E018_BATCH_LOCALES) {
      const produced = await produceMicro039LocaleEpisodeTts({
        ...
        attributionPrefix: "micro-039-e018",
        locale,
        episodeId,
        packRoot,
      ...
      episodes.push({
        locale: locale as Micro039E018BatchLocale,
        episodeId,
  ...
  // Fixture private publication for en-US E018 only (one publication call).
  const enEpisode = episodes.find(
    (entry) => entry.locale === "en-US" && entry.episodeId === "E018"
  );
  ...
  if (enEpisode) {
    const initResult = await input.ports.directPostAdapter.initDirectPost({
      request: {
        ...
        initRequestId: "init.request.micro-039-e018.e018",
        attemptId: MICRO_039_E018_PUBLICATION_ATTEMPT_ID,
        intentId: MICRO_039_E018_PUBLICATION_INTENT_ID,
        idempotencyKey: MICRO_039_E018_PUBLICATION_IDEMPOTENCY_KEY,
        attemptFence: 1,
        binding: {
          provider: "tiktok",
          ...
          episodeId: "e018",
          episodeRevisionId: "rev.script.en-us.e018",
          locale: "en-US",
          renderHash: enEpisode.visualRenderHash,
          metadataRevisionId: "meta.rev.micro-039-e018.e018",
          consentRevisionId: "consent.micro-039-e018.e018",
          exportApprovalRevisionId: "export.approval.micro-039-e018.e018",
          privacy: "private",
          interactionSettings: {
            allowComments: false,
            allowDuet: false,
        ...
        transferPlanId: "transfer.micro-039-e018.e018",
        ...
        providerAccountId: MICRO_050_CANARY_PROVIDER_ACCOUNT_ID,
        credentialVersion: MICRO_050_REFRESHED_CREDENTIAL_VERSION_ID,
        accountFence: "fence.micro-039-e018.1",
    ...
    publishId =
      initResult.kind === "initialized"
        ...
        : null;
    receiptPublicVideoId = "video.micro-039-e018.e018.private";
  }
  ...
  const evidence = {
    schemaVersion: "mediaforge.microdrama.micro-039-e018-batch-execution-evidence.v1",
    taskId: MICRO_039_E018_TASK_ID,
  ...
  microdramaRepository.replaceProjection({
    projectionKey: MICRO_039_E018_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY,
    projection: evidence,
