...
import {
  ...
  type Micro039E016UpstreamEvidence,
} from "./micro-039-e016-batch-evidence.js";
import { SEVEN_MINUTES_AHEAD_SERIES_ID } from "./v5-pack-constants.js";

export const MICRO_039_E016_LEARNING_ADMISSION_ID =
  "admission.micro-039-e016.e016.hook-presentation";

function anchoredSnapshotForE015(): NarrativeSnapshotPayload {
  return {
    schemaVersion: NARRATIVE_SCHEMA_VERSION,
    snapshotId: narrativeSnapshotIdSchema.parse("snapshot.season-1.e015"),
    seriesId: seriesIdSchema.parse(SEVEN_MINUTES_AHEAD_SERIES_ID),
    episodeId: "episode.e015",
    ...
    provenance: {
      ...
      sourceRevisionIds: [
        narrativeRevisionIdSchema.parse("rev.snapshot.season-1.e015"),
      ],
    },
  };
}

export function evaluateMicro039E016LearningAdmission(input: {
  ...
  const recommendation = planCreativeRecommendation({
    ...
    guidance:
      "Tighten the cold-open hook presentation for E016 based on public-video counters.",
    status: "VALIDATED",
    recordedAt: input.evaluatedAt,
  });

  const context = {
    anchoredSnapshot: anchoredSnapshotForE015(),
    anchoredSnapshotRevisionId: "rev.snapshot.season-1.e015",
    lastAcceptedEpisodeNumber: 15,
  };
  ...
  const acceptance = acceptCreativeRecommendationForPlanning({
    recommendation,
    context,
    admissionId: MICRO_039_E016_LEARNING_ADMISSION_ID,
...
export type Micro039E016ProgressiveBatchPreflightInput = {
  readonly evaluatedAt: string;
  ...
  readonly micro036Evidence?: Micro039E016UpstreamEvidence | null;
  readonly micro038Evidence?: Micro039E016UpstreamEvidence | null;
  readonly micro039E015Evidence?: Micro039E016UpstreamEvidence | null;
  readonly micro042Evidence?: Micro039E016UpstreamEvidence | null;
...
export type Micro039E016ProgressiveBatchPreflightResult = {
  readonly preflight: MicrodramaCanaryPreflightResult;
  readonly bindingProbe: ReturnType<typeof buildMicro039E016BatchBindingProbe>;
  readonly learningAdmissionId: string | null;
  readonly rangeAdmission: ReturnType<typeof assertMicro039E016EpisodeRangeAllowed>;
};

export function evaluateMicro039E016ProgressiveBatchPreflight(
  input: Micro039E016ProgressiveBatchPreflightInput
): Micro039E016ProgressiveBatchPreflightResult {
  const bindingProbe =
    input.operatorAuthorization &&
    "episodeRange" in input.operatorAuthorization.bindings
      ? input.operatorAuthorization.bindings
      : buildMicro039E016BatchBindingProbe({ revisionSet: input.revisionSet });

  const rangeAdmission = assertMicro039E016EpisodeRangeAllowed(bindingProbe.episodeRange);
  const learning = evaluateMicro039E016LearningAdmission({
    evaluatedAt: input.evaluatedAt,
  });

  const assetApproved =
    input.assetGenerationApproval?.state === "active" &&
    input.assetGenerationApproval.taskId === MICRO_039_E016_TASK_ID;
  ...
  const readinessGates = [
    ...
    {
      gate: "UPSTREAM_PROGRESSIVE_E015_BATCH_DONE",
      ok: micro039E015EvidenceProvesProgressiveDone(input.micro039E015Evidence ?? null),
      ...(micro039E015EvidenceProvesProgressiveDone(input.micro039E015Evidence ?? null)
        ? {}
        : { message: "MICRO-039 E015 progressive batch evidence missing." }),
    },
    {
...
export { MICRO_039_E016_EPISODE_RANGE };
