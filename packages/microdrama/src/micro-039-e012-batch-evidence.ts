...
import { MICRO_039_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY } from "./micro-039-bounded-progressive-batch-execute.js";
...
export const DEFAULT_MICRO_039_E011_BATCH_EVIDENCE_JSON_PATH =
...
export type Micro039E012UpstreamEvidence = {
...
function loadJsonEvidence(path: string): Micro039E012UpstreamEvidence | null {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Micro039E012UpstreamEvidence;
...
function loadProjectionEvidence(
  ...
  key: string
): Micro039E012UpstreamEvidence | null {
  const stored = repository.getProjection(key);
  if (!stored) {
    return null;
  }
  const projection = stored.projection as Micro039E012UpstreamEvidence & {
...
export function loadMicro036BatchEvidenceForMicro039E012(input: {
...
export function loadMicro038PublicEvidenceForMicro039E012(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E012UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro039E011BatchEvidenceForMicro039E012(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E012UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro042ReadEvidenceForMicro039E012(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E012UpstreamEvidence | null {
  if (input.repository) {
...
export function micro036EvidenceProvesBatchDone(
  evidence: Micro039E012UpstreamEvidence | null
): boolean {
...
export function micro039E011EvidenceProvesProgressiveDone(
  evidence: Micro039E012UpstreamEvidence | null
): boolean {
  ...
  const hasE011 = episodes.some((entry) => {
    const episodeId =
      typeof entry === "object" &&
      entry !== null &&
      "episodeId" in entry &&
      typeof (entry as { episodeId?: unknown }).episodeId === "string"
        ...
        : null;
    return episodeId === "E011";
  });
  const rangeIsE011 =
    evidence.episodeRange?.startEpisodeId?.toLowerCase() === "e011" &&
    evidence.episodeRange?.endEpisodeId?.toLowerCase() === "e011";
  return hasE011 || rangeIsE011 || (evidence.publicationCalls ?? 0) === 1;
}
