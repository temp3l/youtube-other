...
import {
  buildMicro039OperatorAuthorizationRecord,
  computeMicro039AuthorizationRevisionId,
  loadMicro039AssetGenerationApproval,
  loadMicro039CostBudgetApproval,
  loadMicro039OperatorAuthorization,
  MICRO_039_ASSET_GENERATION_APPROVAL_ID,
  MICRO_039_COST_BUDGET_APPROVAL_ID,
  persistMicro039AssetGenerationApproval,
  persistMicro039CostBudgetApproval,
  persistMicro039OperatorAuthorization,
  type Micro039CostBudgetApprovalRecord,
} from "./micro-039-batch-authorization-persistence.js";
import {
  MICRO_039_BATCH_COST_LIMIT_MINOR,
  MICRO_039_BATCH_CURRENCY,
  MICRO_039_BATCH_EPISODE_IDS,
  MICRO_039_BATCH_LOCALES,
  MICRO_039_BATCH_MAXIMUM_TOTAL_PROVIDER_REQUESTS,
  MICRO_039_PRODUCTION_PROVIDERS,
  MICRO_039_TASK_ID,
  MICRO_039_VISUAL_PROFILE_REVISION,
  computeMicro039ProviderConfigRevision,
...
export type Micro039BoundedProgressiveBatchAuthorizationPreparationInput = {
...
export type Micro039BoundedProgressiveBatchAuthorizationPreparationResult = {
...
function buildAssetGenerationApproval(input: {
  ...
  return {
    ...
    approvalId: MICRO_039_ASSET_GENERATION_APPROVAL_ID,
    ...
    scope: {
      ...
      voiceRevision: MICRO_039_VISUAL_PROFILE_REVISION,
...
function buildCostBudgetApproval(input: {
  ...
  readonly operatorId: string;
  readonly approvedAt: string;
}): Micro039CostBudgetApprovalRecord {
  return {
    schemaVersion: "mediaforge.microdrama-cost-budget-approval.v1",
    approvalId: MICRO_039_COST_BUDGET_APPROVAL_ID,
    taskId: MICRO_039_TASK_ID,
    costLimitMinor: MICRO_039_BATCH_COST_LIMIT_MINOR,
    currency: MICRO_039_BATCH_CURRENCY,
...
export async function prepareMicro039BoundedProgressiveBatchAuthorization(
  input: Micro039BoundedProgressiveBatchAuthorizationPreparationInput
): Promise<Micro039BoundedProgressiveBatchAuthorizationPreparationResult> {
  const blockers: string[] = [];
  const packRoot = input.packRoot ?? defaultV5PackRoot;
  const operatorId = input.operatorId ?? "operator.microdrama";
  const providerConfigRevision = computeMicro039ProviderConfigRevision();
  ...
  if (blockers.length === 0) {
    persistMicro039OperatorAuthorization({
      repository: microdramaRepository,
      record: buildMicro039OperatorAuthorizationRecord({
        revisionSet,
        operatorId,
  ...
  const finalPreflight = evaluateMicro039ProgressiveBatchPreflight({
...
export function micro039AuthorizationEvidenceSummary(input: {
  readonly preparation: Micro039BoundedProgressiveBatchAuthorizationPreparationResult;
