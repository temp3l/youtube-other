...
import { MICRO_039_E013_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY } from "./micro-039-e013-bounded-progressive-batch-execute.js";
...
export const DEFAULT_MICRO_039_E013_BATCH_EVIDENCE_JSON_PATH =
...
export type Micro039E014UpstreamEvidence = {
...
function loadJsonEvidence(path: string): Micro039E014UpstreamEvidence | null {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Micro039E014UpstreamEvidence;
...
function loadProjectionEvidence(
  ...
  key: string
): Micro039E014UpstreamEvidence | null {
  const stored = repository.getProjection(key);
  if (!stored) {
    return null;
  }
  const projection = stored.projection as Micro039E014UpstreamEvidence & {
...
export function loadMicro036BatchEvidenceForMicro039E014(input: {
...
export function loadMicro038PublicEvidenceForMicro039E014(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E014UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro039E013BatchEvidenceForMicro039E014(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E014UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro042ReadEvidenceForMicro039E014(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E014UpstreamEvidence | null {
  if (input.repository) {
...
export function micro036EvidenceProvesBatchDone(
  evidence: Micro039E014UpstreamEvidence | null
): boolean {
...
export function micro039E013EvidenceProvesProgressiveDone(
  evidence: Micro039E014UpstreamEvidence | null
): boolean {
  ...
  const hasE013 = episodes.some((entry) => {
    const episodeId =
      typeof entry === "object" &&
      entry !== null &&
      "episodeId" in entry &&
      typeof (entry as { episodeId?: unknown }).episodeId === "string"
        ...
        : null;
    return episodeId === "E013";
  });
  const rangeIsE013 =
    evidence.episodeRange?.startEpisodeId?.toLowerCase() === "e013" &&
    evidence.episodeRange?.endEpisodeId?.toLowerCase() === "e013";
  return hasE013 || rangeIsE013 || (evidence.publicationCalls ?? 0) === 1;
}
