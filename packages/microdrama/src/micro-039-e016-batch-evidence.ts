...
import { MICRO_039_E015_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY } from "./micro-039-e015-bounded-progressive-batch-execute.js";
...
export const DEFAULT_MICRO_039_E015_BATCH_EVIDENCE_JSON_PATH =
...
export type Micro039E016UpstreamEvidence = {
...
function loadJsonEvidence(path: string): Micro039E016UpstreamEvidence | null {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Micro039E016UpstreamEvidence;
...
function loadProjectionEvidence(
  ...
  key: string
): Micro039E016UpstreamEvidence | null {
  const stored = repository.getProjection(key);
  if (!stored) {
    return null;
  }
  const projection = stored.projection as Micro039E016UpstreamEvidence & {
...
export function loadMicro036BatchEvidenceForMicro039E016(input: {
...
export function loadMicro038PublicEvidenceForMicro039E016(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E016UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro039E015BatchEvidenceForMicro039E016(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E016UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro042ReadEvidenceForMicro039E016(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E016UpstreamEvidence | null {
  if (input.repository) {
...
export function micro036EvidenceProvesBatchDone(
  evidence: Micro039E016UpstreamEvidence | null
): boolean {
...
export function micro039E015EvidenceProvesProgressiveDone(
  evidence: Micro039E016UpstreamEvidence | null
): boolean {
  ...
  const hasE015 = episodes.some((entry) => {
    const episodeId =
      typeof entry === "object" &&
      entry !== null &&
      "episodeId" in entry &&
      typeof (entry as { episodeId?: unknown }).episodeId === "string"
        ...
        : null;
    return episodeId === "E015";
  });
  const rangeIsE015 =
    evidence.episodeRange?.startEpisodeId?.toLowerCase() === "e015" &&
    evidence.episodeRange?.endEpisodeId?.toLowerCase() === "e015";
  return hasE015 || rangeIsE015 || (evidence.publicationCalls ?? 0) === 1;
}
