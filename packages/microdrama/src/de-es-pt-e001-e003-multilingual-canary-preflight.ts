...
export const MICRO_035_TASK_ID = "MICRO-035";
export const DE_ES_PT_E001_E003_MULTILINGUAL_CANARY_EPISODES = MICRO_035_CANARY_EPISODE_IDS;
...
export type DeEsPtE001E003MultilingualCanaryPreflightInput = {
  readonly packRoot: string;
  readonly admittedAt: string;
  readonly evaluatedAt: string;
  readonly profiles: readonly MicrodramaBudgetProfile[];
  readonly micro034Evidence?: Micro034CanaryExecutionEvidence | null;
...
export type DeEsPtE001E003MultilingualCanaryPreflightResult = {
...
function resolveModelConfigurationForLocale(
  ...
  return resolveMicro035OpenAiTtsModelConfigurationForLocale(locale);
}

export async function evaluateDeEsPtE001E003MultilingualCanaryPreflight(
  input: DeEsPtE001E003MultilingualCanaryPreflightInput
): Promise<DeEsPtE001E003MultilingualCanaryPreflightResult> {
  const episodeCostAllocations = resolveMicro035CanaryEpisodeCostMinorAllocations();
  const micro034Evidence =
    input.micro034Evidence ??
    loadMicro034CanaryExecutionEvidence({
  ...
  const primaryLocale = "de-DE";

  const sharedVisualsApproved =
    micro034Evidence?.status === "DONE" &&
    micro034Evidence.episodes.length === DE_ES_PT_E001_E003_MULTILINGUAL_CANARY_EPISODES.length &&
    DE_ES_PT_E001_E003_MULTILINGUAL_CANARY_EPISODES.every((episodeId) =>
      micro034Evidence.episodes.some((episode) => episode.episodeId === episodeId)
    );
  readinessGates.push({
    gate: "SHARED_VISUALS_APPROVED",
    ok: sharedVisualsApproved,
    ...(sharedVisualsApproved
      ? {}
      : { message: "MICRO-034 canary execution evidence is missing or incomplete." }),
  });

  for (const locale of MICRO_035_CANARY_LOCALES) {
    const modelConfiguration = resolveModelConfigurationForLocale(locale);

    for (const episodeId of DE_ES_PT_E001_E003_MULTILINGUAL_CANARY_EPISODES) {
      ...
      const audioBudgetPreflight = evaluateMicrodramaBudgetPreflight({
        correlationId: `corr.canary.micro-035.audio.${locale.toLowerCase()}.${episodeId.toLowerCase()}`,
        workItems: [
          {
      ...
      const trustDecision = evaluateTrustGate({
        correlationId: `corr.canary.micro-035.trust.${locale.toLowerCase()}.${episodeId.toLowerCase()}`,
        evaluatedAt: input.evaluatedAt,
        untrustedPayload: {
          episodeId,
          locale: script.locale,
      ...
      const visualBudgetPreflight = evaluateVisualRenderBudgetPreflight({
        binding: compiledVisual.binding,
        profiles: input.profiles,
        correlationId: `corr.canary.micro-035.visual.${locale.toLowerCase()}.${episodeId.toLowerCase()}`,
        evaluatedAt: input.evaluatedAt,
  ...
  const aggregateBudgetPreflight = evaluateMicrodramaBudgetPreflight({
    correlationId: "corr.canary.micro-035",
    workItems,
    profiles: input.profiles,
    commitments: [],
    evaluatedAt: input.evaluatedAt,
  ...
  const bindingProbe: BoundedPaidProviderBindingProbe = {
    episodeIds: [...DE_ES_PT_E001_E003_MULTILINGUAL_CANARY_EPISODES],
  ...
  const preflight = evaluateBoundedPaidProviderCanaryPreflight({
    ...
    bindingProbe,
    requiredAssetKinds: ["tts", "alignment", "render"],
    permittedCallPolicy: {
...
export function defaultMultilingualCanaryBudgetProfiles(
  evaluatedAt: string
): MicrodramaBudgetProfile[] {
  const profile = (
