...
import { assertMicro039E018EpisodeRangeAllowed } from "./micro-039-e018-batch-bindings.js";
import {
...
import { prepareMicro039E018BoundedProgressiveBatchAuthorization } from "./micro-039-e018-bounded-progressive-batch-authorization-preparation.js";
import {
...
describe("MICRO-039-E018 progressive E018 batch", () => {
  ...
  it("authorizes and executes separately authorized progressive E018 batch from upstream evidence", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "mediaforge-micro-039-e018-"));
    const dbPath = path.join(dir, "microdrama.sqlite");
    const outputRoot = path.join(dir, "output");

    const preparation = await prepareMicro039E018BoundedProgressiveBatchAuthorization({
      dbPath,
      ...
      micro039E017EvidenceJsonPath: DEFAULT_MICRO_039_E017_BATCH_EVIDENCE_JSON_PATH,
    ...
    const ports = createMicro039E018ProgressiveBatchPorts();
    const authorization = await authorizeMicro039E018BoundedProgressiveBatchExplicitExecute({
      dbPath,
    ...
    const execution = await executeMicro039E018BoundedProgressiveBatch({
      dbPath,
    ...
    expect(execution.episodes.every((episode) => episode.episodeId === "E018")).toBe(
      true
    );
    ...
    expect(execution.publishId).toMatch(/^tiktok\.publish\.micro-039-e018\./);
    expect(execution.observationAudit.status).toBe("PASSED");

    const blockedOutOfScope = await executeMicro039E018BoundedProgressiveBatch({
      dbPath,
      executedAt: EXECUTED_AT,
      outputRoot: path.join(dir, "blocked"),
      ports: createMicro039E018ProgressiveBatchPorts(),
      segmentSynthesisPorts: mockSegmentPorts(),
      measureDurationMs: () => 120,
      skipNarrationConcat: true,
      requestedEpisodeIds: ["E019"],
      micro036EvidenceJsonPath: DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
      micro038EvidenceJsonPath: DEFAULT_MICRO_038_PUBLIC_EVIDENCE_JSON_PATH,
      micro039E017EvidenceJsonPath: DEFAULT_MICRO_039_E017_BATCH_EVIDENCE_JSON_PATH,
      micro042EvidenceJsonPath: DEFAULT_MICRO_042_READ_EVIDENCE_JSON_PATH,
    ...
    expect(
      blockedOutOfScope.blockers.some((blocker) => blocker.includes("E019"))
    ).toBe(true);
    ...
    const stored = repo.getProjection(MICRO_039_E018_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY);
    expect(stored?.projection.publicationCalls).toBe(1);
  }, 180_000);

  it.runIf(process.env.MICRO_039_E018_OPERATOR_PREP === "1")(
    "prepares workspace operator embedded database",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath = path.join(repoRoot, ".mediaforge.sqlite");
      const preparedAt = process.env.MICRO_039_E018_PREPARED_AT ?? new Date().toISOString();

      const result = await prepareMicro039E018BoundedProgressiveBatchAuthorization({
        dbPath,
        admittedAt: ADMITTED_AT,
        preparedAt,
        micro036EvidenceJsonPath: DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e018-authorization-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e018-authorization-evidence.v1",
            taskId: "MICRO-039-E018",
            dbPath,
            preparedAt,
            ...result,
          },
  ...
  it.runIf(process.env.MICRO_039_E018_OPERATOR_EXECUTE === "1")(
    "executes workspace operator progressive E018 batch",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath =
        process.env.MICRO_039_E018_DB_PATH ?? path.join(repoRoot, ".mediaforge.sqlite");
      const executedAt = process.env.MICRO_039_E018_EXECUTED_AT ?? new Date().toISOString();
      const outputRoot = path.join(repoRoot, ".artifacts/microdrama/micro-039-e018-batch");
      const ports = createMicro039E018ProgressiveBatchPorts();
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e018-batch-execution-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e018-batch-execution-evidence.v1",
            dbPath,
            executedAt,
  ...
  it.runIf(process.env.MICRO_039_E018_LIVE_TTS === "1")(
    ...
    async () => {
      ...
      const outputRoot = path.join(
        repoRoot,
        ".artifacts/microdrama/micro-039-e018-live-tts-batch"
      );
      const ports = createMicro039E018ProgressiveBatchPorts();
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e018-live-tts-batch-execution-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e018-live-tts-batch-execution-evidence.v1",
            dbPath,
            executedAt,
            outputRoot,
            publicationCalls: execution.publicationCalls,
