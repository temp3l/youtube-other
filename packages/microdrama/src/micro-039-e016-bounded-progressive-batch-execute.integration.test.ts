...
import { assertMicro039E016EpisodeRangeAllowed } from "./micro-039-e016-batch-bindings.js";
import {
...
import { prepareMicro039E016BoundedProgressiveBatchAuthorization } from "./micro-039-e016-bounded-progressive-batch-authorization-preparation.js";
import {
...
describe("MICRO-039-E016 progressive E016 batch", () => {
  ...
  it("authorizes and executes separately authorized progressive E016 batch from upstream evidence", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "mediaforge-micro-039-e016-"));
    const dbPath = path.join(dir, "microdrama.sqlite");
    const outputRoot = path.join(dir, "output");

    const preparation = await prepareMicro039E016BoundedProgressiveBatchAuthorization({
      dbPath,
      ...
      micro039E015EvidenceJsonPath: DEFAULT_MICRO_039_E015_BATCH_EVIDENCE_JSON_PATH,
    ...
    const ports = createMicro039E016ProgressiveBatchPorts();
    const authorization = await authorizeMicro039E016BoundedProgressiveBatchExplicitExecute({
      dbPath,
    ...
    const execution = await executeMicro039E016BoundedProgressiveBatch({
      dbPath,
    ...
    expect(execution.episodes.every((episode) => episode.episodeId === "E016")).toBe(
      true
    );
    ...
    expect(execution.publishId).toMatch(/^tiktok\.publish\.micro-039-e016\./);
    expect(execution.observationAudit.status).toBe("PASSED");

    const blockedOutOfScope = await executeMicro039E016BoundedProgressiveBatch({
      dbPath,
      executedAt: EXECUTED_AT,
      outputRoot: path.join(dir, "blocked"),
      ports: createMicro039E016ProgressiveBatchPorts(),
      segmentSynthesisPorts: mockSegmentPorts(),
      measureDurationMs: () => 120,
      skipNarrationConcat: true,
      requestedEpisodeIds: ["E017"],
      micro036EvidenceJsonPath: DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
      micro038EvidenceJsonPath: DEFAULT_MICRO_038_PUBLIC_EVIDENCE_JSON_PATH,
      micro039E015EvidenceJsonPath: DEFAULT_MICRO_039_E015_BATCH_EVIDENCE_JSON_PATH,
      micro042EvidenceJsonPath: DEFAULT_MICRO_042_READ_EVIDENCE_JSON_PATH,
    ...
    expect(
      blockedOutOfScope.blockers.some((blocker) => blocker.includes("E017"))
    ).toBe(true);
    ...
    const stored = repo.getProjection(MICRO_039_E016_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY);
    expect(stored?.projection.publicationCalls).toBe(1);
  }, 180_000);

  it.runIf(process.env.MICRO_039_E016_OPERATOR_PREP === "1")(
    "prepares workspace operator embedded database",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath = path.join(repoRoot, ".mediaforge.sqlite");
      const preparedAt = process.env.MICRO_039_E016_PREPARED_AT ?? new Date().toISOString();

      const result = await prepareMicro039E016BoundedProgressiveBatchAuthorization({
        dbPath,
        admittedAt: ADMITTED_AT,
        preparedAt,
        micro036EvidenceJsonPath: DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e016-authorization-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e016-authorization-evidence.v1",
            taskId: "MICRO-039-E016",
            dbPath,
            preparedAt,
            ...result,
          },
  ...
  it.runIf(process.env.MICRO_039_E016_OPERATOR_EXECUTE === "1")(
    "executes workspace operator progressive E016 batch",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath =
        process.env.MICRO_039_E016_DB_PATH ?? path.join(repoRoot, ".mediaforge.sqlite");
      const executedAt = process.env.MICRO_039_E016_EXECUTED_AT ?? new Date().toISOString();
      const outputRoot = path.join(repoRoot, ".artifacts/microdrama/micro-039-e016-batch");
      const ports = createMicro039E016ProgressiveBatchPorts();
      ...
      writeFileSync(
        ...
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e016-batch-execution-evidence.v1",
            dbPath,
            executedAt,
  ...
  it.runIf(process.env.MICRO_039_E016_LIVE_TTS === "1")(
    "executes workspace operator progressive E016 batch with live OpenAI TTS",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath =
        process.env.MICRO_039_E016_DB_PATH ?? path.join(repoRoot, ".mediaforge.sqlite");
      const executedAt =
        process.env.MICRO_039_E016_EXECUTED_AT ?? new Date().toISOString();
      const outputRoot = path.join(
        repoRoot,
        ".artifacts/microdrama/micro-039-e016-live-tts-batch"
      );
      const ports = createMicro039E016ProgressiveBatchPorts();
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e016-live-tts-batch-execution-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e016-live-tts-batch-execution-evidence.v1",
            dbPath,
            executedAt,
            outputRoot,
            publicationCalls: execution.publicationCalls,
