...
import { MICRO_039_E012_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY } from "./micro-039-e012-bounded-progressive-batch-execute.js";
...
export const DEFAULT_MICRO_039_E012_BATCH_EVIDENCE_JSON_PATH =
...
export type Micro039E013UpstreamEvidence = {
...
function loadJsonEvidence(path: string): Micro039E013UpstreamEvidence | null {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Micro039E013UpstreamEvidence;
...
function loadProjectionEvidence(
  ...
  key: string
): Micro039E013UpstreamEvidence | null {
  const stored = repository.getProjection(key);
  if (!stored) {
    return null;
  }
  const projection = stored.projection as Micro039E013UpstreamEvidence & {
...
export function loadMicro036BatchEvidenceForMicro039E013(input: {
...
export function loadMicro038PublicEvidenceForMicro039E013(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E013UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro039E012BatchEvidenceForMicro039E013(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E013UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro042ReadEvidenceForMicro039E013(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E013UpstreamEvidence | null {
  if (input.repository) {
...
export function micro036EvidenceProvesBatchDone(
  evidence: Micro039E013UpstreamEvidence | null
): boolean {
...
export function micro039E012EvidenceProvesProgressiveDone(
  evidence: Micro039E013UpstreamEvidence | null
): boolean {
  ...
  const hasE012 = episodes.some((entry) => {
    const episodeId =
      typeof entry === "object" &&
      entry !== null &&
      "episodeId" in entry &&
      typeof (entry as { episodeId?: unknown }).episodeId === "string"
        ...
        : null;
    return episodeId === "E012";
  });
  const rangeIsE012 =
    evidence.episodeRange?.startEpisodeId?.toLowerCase() === "e012" &&
    evidence.episodeRange?.endEpisodeId?.toLowerCase() === "e012";
  return hasE012 || rangeIsE012 || (evidence.publicationCalls ?? 0) === 1;
}
