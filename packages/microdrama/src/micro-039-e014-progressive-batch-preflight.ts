...
import {
  ...
  type Micro039E014UpstreamEvidence,
} from "./micro-039-e014-batch-evidence.js";
import { SEVEN_MINUTES_AHEAD_SERIES_ID } from "./v5-pack-constants.js";

export const MICRO_039_E014_LEARNING_ADMISSION_ID =
  "admission.micro-039-e014.e014.hook-presentation";

function anchoredSnapshotForE013(): NarrativeSnapshotPayload {
  return {
    schemaVersion: NARRATIVE_SCHEMA_VERSION,
    snapshotId: narrativeSnapshotIdSchema.parse("snapshot.season-1.e013"),
    seriesId: seriesIdSchema.parse(SEVEN_MINUTES_AHEAD_SERIES_ID),
    episodeId: "episode.e013",
    ...
    provenance: {
      ...
      sourceRevisionIds: [
        narrativeRevisionIdSchema.parse("rev.snapshot.season-1.e013"),
      ],
    },
  };
}

export function evaluateMicro039E014LearningAdmission(input: {
  ...
  const recommendation = planCreativeRecommendation({
    ...
    guidance:
      "Tighten the cold-open hook presentation for E014 based on public-video counters.",
    status: "VALIDATED",
    recordedAt: input.evaluatedAt,
  });

  const context = {
    anchoredSnapshot: anchoredSnapshotForE013(),
    anchoredSnapshotRevisionId: "rev.snapshot.season-1.e013",
    lastAcceptedEpisodeNumber: 13,
  };
  ...
  const acceptance = acceptCreativeRecommendationForPlanning({
    recommendation,
    context,
    admissionId: MICRO_039_E014_LEARNING_ADMISSION_ID,
...
export type Micro039E014ProgressiveBatchPreflightInput = {
  readonly evaluatedAt: string;
  ...
  readonly micro036Evidence?: Micro039E014UpstreamEvidence | null;
  readonly micro038Evidence?: Micro039E014UpstreamEvidence | null;
  readonly micro039E013Evidence?: Micro039E014UpstreamEvidence | null;
  readonly micro042Evidence?: Micro039E014UpstreamEvidence | null;
...
export type Micro039E014ProgressiveBatchPreflightResult = {
  readonly preflight: MicrodramaCanaryPreflightResult;
  readonly bindingProbe: ReturnType<typeof buildMicro039E014BatchBindingProbe>;
  readonly learningAdmissionId: string | null;
  readonly rangeAdmission: ReturnType<typeof assertMicro039E014EpisodeRangeAllowed>;
};

export function evaluateMicro039E014ProgressiveBatchPreflight(
  input: Micro039E014ProgressiveBatchPreflightInput
): Micro039E014ProgressiveBatchPreflightResult {
  const bindingProbe =
    input.operatorAuthorization &&
    "episodeRange" in input.operatorAuthorization.bindings
      ? input.operatorAuthorization.bindings
      : buildMicro039E014BatchBindingProbe({ revisionSet: input.revisionSet });

  const rangeAdmission = assertMicro039E014EpisodeRangeAllowed(bindingProbe.episodeRange);
  const learning = evaluateMicro039E014LearningAdmission({
    evaluatedAt: input.evaluatedAt,
  });

  const assetApproved =
    input.assetGenerationApproval?.state === "active" &&
    input.assetGenerationApproval.taskId === MICRO_039_E014_TASK_ID;
  ...
  const readinessGates = [
    ...
    {
      gate: "UPSTREAM_PROGRESSIVE_E013_BATCH_DONE",
      ok: micro039E013EvidenceProvesProgressiveDone(input.micro039E013Evidence ?? null),
      ...(micro039E013EvidenceProvesProgressiveDone(input.micro039E013Evidence ?? null)
        ? {}
        : { message: "MICRO-039 E013 progressive batch evidence missing." }),
    },
    {
...
export { MICRO_039_E014_EPISODE_RANGE };
