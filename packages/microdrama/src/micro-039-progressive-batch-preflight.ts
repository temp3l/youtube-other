...
import {
  ...
  type Micro039UpstreamEvidence,
} from "./micro-039-batch-evidence.js";
import { SEVEN_MINUTES_AHEAD_SERIES_ID } from "./v5-pack-constants.js";

export const MICRO_039_LEARNING_ADMISSION_ID =
  "admission.micro-039.e011.hook-presentation";

function anchoredSnapshotForE010(): NarrativeSnapshotPayload {
  return {
    schemaVersion: NARRATIVE_SCHEMA_VERSION,
    snapshotId: narrativeSnapshotIdSchema.parse("snapshot.season-1.e010"),
    ...
    provenance: {
      ...
      sourceRevisionIds: [
        narrativeRevisionIdSchema.parse("rev.snapshot.season-1.e010"),
      ],
    },
  };
}

export function evaluateMicro039LearningAdmission(input: {
  ...
  const recommendation = planCreativeRecommendation({
    recommendationId: "recommendation.micro-039.e011.hook",
    revisionNumber: 1,
    ...
    guidance:
      "Tighten the cold-open hook presentation for E011 based on public-video counters.",
    status: "VALIDATED",
    recordedAt: input.evaluatedAt,
  });

  const context = {
    anchoredSnapshot: anchoredSnapshotForE010(),
    anchoredSnapshotRevisionId: "rev.snapshot.season-1.e010",
  ...
  const acceptance = acceptCreativeRecommendationForPlanning({
    recommendation,
    context,
    admissionId: MICRO_039_LEARNING_ADMISSION_ID,
...
export type Micro039ProgressiveBatchPreflightInput = {
  readonly evaluatedAt: string;
  ...
  readonly micro036Evidence?: Micro039UpstreamEvidence | null;
  readonly micro038Evidence?: Micro039UpstreamEvidence | null;
  readonly micro042Evidence?: Micro039UpstreamEvidence | null;
...
export type Micro039ProgressiveBatchPreflightResult = {
  readonly preflight: MicrodramaCanaryPreflightResult;
  readonly bindingProbe: ReturnType<typeof buildMicro039BatchBindingProbe>;
  readonly learningAdmissionId: string | null;
  readonly rangeAdmission: ReturnType<typeof assertMicro039EpisodeRangeAllowed>;
};

export function evaluateMicro039ProgressiveBatchPreflight(
  input: Micro039ProgressiveBatchPreflightInput
): Micro039ProgressiveBatchPreflightResult {
  const bindingProbe =
    input.operatorAuthorization &&
    "episodeRange" in input.operatorAuthorization.bindings
      ? input.operatorAuthorization.bindings
      : buildMicro039BatchBindingProbe({ revisionSet: input.revisionSet });

  const rangeAdmission = assertMicro039EpisodeRangeAllowed(bindingProbe.episodeRange);
  const learning = evaluateMicro039LearningAdmission({
    evaluatedAt: input.evaluatedAt,
  });

  const assetApproved =
    input.assetGenerationApproval?.state === "active" &&
    input.assetGenerationApproval.taskId === MICRO_039_TASK_ID;
...
export { MICRO_039_EPISODE_RANGE };
