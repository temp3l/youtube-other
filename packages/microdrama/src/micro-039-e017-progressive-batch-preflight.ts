...
import {
  ...
  type Micro039E017UpstreamEvidence,
} from "./micro-039-e017-batch-evidence.js";
import { SEVEN_MINUTES_AHEAD_SERIES_ID } from "./v5-pack-constants.js";

export const MICRO_039_E017_LEARNING_ADMISSION_ID =
  "admission.micro-039-e017.e017.hook-presentation";

function anchoredSnapshotForE016(): NarrativeSnapshotPayload {
  return {
    schemaVersion: NARRATIVE_SCHEMA_VERSION,
    snapshotId: narrativeSnapshotIdSchema.parse("snapshot.season-1.e016"),
    seriesId: seriesIdSchema.parse(SEVEN_MINUTES_AHEAD_SERIES_ID),
    episodeId: "episode.e016",
    ...
    provenance: {
      ...
      sourceRevisionIds: [
        narrativeRevisionIdSchema.parse("rev.snapshot.season-1.e016"),
      ],
    },
  };
}

export function evaluateMicro039E017LearningAdmission(input: {
  ...
  const recommendation = planCreativeRecommendation({
    ...
    guidance:
      "Tighten the cold-open hook presentation for E017 based on public-video counters.",
    status: "VALIDATED",
    recordedAt: input.evaluatedAt,
  });

  const context = {
    anchoredSnapshot: anchoredSnapshotForE016(),
    anchoredSnapshotRevisionId: "rev.snapshot.season-1.e016",
    lastAcceptedEpisodeNumber: 16,
  };
  ...
  const acceptance = acceptCreativeRecommendationForPlanning({
    recommendation,
    context,
    admissionId: MICRO_039_E017_LEARNING_ADMISSION_ID,
...
export type Micro039E017ProgressiveBatchPreflightInput = {
  readonly evaluatedAt: string;
  ...
  readonly micro036Evidence?: Micro039E017UpstreamEvidence | null;
  readonly micro038Evidence?: Micro039E017UpstreamEvidence | null;
  readonly micro039E016Evidence?: Micro039E017UpstreamEvidence | null;
  readonly micro042Evidence?: Micro039E017UpstreamEvidence | null;
...
export type Micro039E017ProgressiveBatchPreflightResult = {
  readonly preflight: MicrodramaCanaryPreflightResult;
  readonly bindingProbe: ReturnType<typeof buildMicro039E017BatchBindingProbe>;
  readonly learningAdmissionId: string | null;
  readonly rangeAdmission: ReturnType<typeof assertMicro039E017EpisodeRangeAllowed>;
};

export function evaluateMicro039E017ProgressiveBatchPreflight(
  input: Micro039E017ProgressiveBatchPreflightInput
): Micro039E017ProgressiveBatchPreflightResult {
  const bindingProbe =
    input.operatorAuthorization &&
    "episodeRange" in input.operatorAuthorization.bindings
      ? input.operatorAuthorization.bindings
      : buildMicro039E017BatchBindingProbe({ revisionSet: input.revisionSet });

  const rangeAdmission = assertMicro039E017EpisodeRangeAllowed(bindingProbe.episodeRange);
  const learning = evaluateMicro039E017LearningAdmission({
    evaluatedAt: input.evaluatedAt,
  });

  const assetApproved =
    input.assetGenerationApproval?.state === "active" &&
    input.assetGenerationApproval.taskId === MICRO_039_E017_TASK_ID;
  ...
  const readinessGates = [
    ...
    {
      gate: "UPSTREAM_PROGRESSIVE_E016_BATCH_DONE",
      ok: micro039E016EvidenceProvesProgressiveDone(input.micro039E016Evidence ?? null),
      ...(micro039E016EvidenceProvesProgressiveDone(input.micro039E016Evidence ?? null)
        ? {}
        : { message: "MICRO-039 E016 progressive batch evidence missing." }),
    },
    {
...
export { MICRO_039_E017_EPISODE_RANGE };
