...
import {
  ...
  isMicro039E017OutOfScopeEpisodeId,
  ...
  MICRO_039_E017_EPISODE_RANGE,
  MICRO_039_E017_PUBLICATION_ATTEMPT_ID,
  MICRO_039_E017_PUBLICATION_IDEMPOTENCY_KEY,
  MICRO_039_E017_PUBLICATION_INTENT_ID,
  MICRO_039_E017_TASK_ID,
  type Micro039E017BatchLocale,
...
import {
  buildMicro039E017ExplicitExecuteAuthorizationRecord,
  computeMicro039E017PreparationFingerprint,
  loadMicro039E017ExplicitExecuteAuthorization,
  persistMicro039E017ExplicitExecuteAuthorization,
  summarizeMicro039E017PreparationForFingerprint,
} from "./micro-039-e017-explicit-execute-authorization.js";
import { evaluateMicro039E017ProgressiveBatchPreflight } from "./micro-039-e017-progressive-batch-preflight.js";

export const MICRO_039_E017_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY =
  "microdrama.batch-execution-evidence.MICRO-039-E017";

export type Micro039E017ProgressiveBatchPorts = {
...
export function createMicro039E017ProgressiveBatchPorts(): Micro039E017ProgressiveBatchPorts {
  ...
  return {
    ...
    directPostAdapter: new FixtureTikTokDirectPostAdapter("tiktok.publish.micro-039-e017"),
    auditRepository,
  };
}

export type Micro039E017EpisodeBatchEvidence = {
  readonly locale: Micro039E017BatchLocale;
  readonly episodeId: "E017";
...
export type Micro039E017AuthorizeExplicitExecuteInput = {
...
export type Micro039E017AuthorizeExplicitExecuteResult = {
...
export type Micro039E017BoundedProgressiveBatchExecuteInput = {
  readonly dbPath: string;
  ...
  readonly ports: Micro039E017ProgressiveBatchPorts;
...
export type Micro039E017BoundedProgressiveBatchExecuteResult = {
  ...
  readonly episodes: readonly Micro039E017EpisodeBatchEvidence[];
...
export async function authorizeMicro039E017BoundedProgressiveBatchExplicitExecute(
  input: Micro039E017AuthorizeExplicitExecuteInput
): Promise<Micro039E017AuthorizeExplicitExecuteResult> {
  ...
  const { micro036Evidence, micro038Evidence, micro039E016Evidence, micro042Evidence } =
    loadUpstream({
    ...
    ...(input.micro038EvidenceJsonPath
      ...
      : {}),
    ...(input.micro039E016EvidenceJsonPath
      ? { micro039E016EvidenceJsonPath: input.micro039E016EvidenceJsonPath }
      : {}),
  ...
  const preflight = evaluateMicro039E017ProgressiveBatchPreflight({
  ...
  const bindingProbe = buildMicro039E017BatchBindingProbe({ revisionSet });
  const preparationSummary = summarizeMicro039E017PreparationForFingerprint(
    microdramaRepository,
    {
      micro036EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e017.micro-036",
        micro036Evidence
      ),
      micro038EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e017.micro-038",
        micro038Evidence
      ),
      micro039E016EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e017.micro-039-e016",
        micro039E016Evidence
      ),
      micro042EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e017.micro-042",
        micro042Evidence
      ),
  ...
  const preparationFingerprint = computeMicro039E017PreparationFingerprint(
    preparationSummary
  );

  const record = buildMicro039E017ExplicitExecuteAuthorizationRecord({
    preparationFingerprint,
    binds: {
      episodeRange: { ...MICRO_039_E017_EPISODE_RANGE },
...
export async function executeMicro039E017BoundedProgressiveBatch(
  input: Micro039E017BoundedProgressiveBatchExecuteInput
): Promise<Micro039E017BoundedProgressiveBatchExecuteResult> {
  const blockers: string[] = [];
  const operatorId = input.operatorId ?? "operator.microdrama";
  const packRoot = input.packRoot ?? defaultV5PackRoot;
  ...
  const executeAuthorization =
    loadMicro039E017ExplicitExecuteAuthorization(microdramaRepository);
  const operatorAuthorization = loadMicro039E017OperatorAuthorization(microdramaRepository);
  const assetGenerationApproval =
    loadMicro039E017AssetGenerationApproval(microdramaRepository);
  const costBudgetApproval = loadMicro039E017CostBudgetApproval(microdramaRepository);
  ...
  for (const episodeId of input.requestedEpisodeIds ?? MICRO_039_E017_BATCH_EPISODE_IDS) {
    if (isMicro039E017OutOfScopeEpisodeId(episodeId)) {
  ...
  if (blockers.length > 0) {
    return {
      ...
      evidenceProjectionKey: MICRO_039_E017_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY,
    };
  }
  ...
  for (const profile of defaultMicro039BudgetProfiles({
    evaluatedAt: input.executedAt,
    episodeIds: MICRO_039_E017_BATCH_EPISODE_IDS,
  })) {
  ...
  const estimatedCostMinor = Math.floor(
    MICRO_039_E017_BATCH_COST_LIMIT_MINOR / MICRO_039_E017_BATCH_LOCALES.length
  );
  ...
  const episodes: Micro039E017EpisodeBatchEvidence[] = [];
  ...
  for (const episodeId of MICRO_039_E017_BATCH_EPISODE_IDS) {
    for (const locale of MICRO_039_E017_BATCH_LOCALES) {
      const produced = await produceMicro039LocaleEpisodeTts({
        ...
        attributionPrefix: "micro-039-e017",
        locale,
        episodeId,
        packRoot,
      ...
      episodes.push({
        locale: locale as Micro039E017BatchLocale,
        episodeId,
  ...
  // Fixture private publication for en-US E017 only (one publication call).
  const enEpisode = episodes.find(
    (entry) => entry.locale === "en-US" && entry.episodeId === "E017"
  );
  ...
  if (enEpisode) {
    const initResult = await input.ports.directPostAdapter.initDirectPost({
      request: {
        ...
        initRequestId: "init.request.micro-039-e017.e017",
        attemptId: MICRO_039_E017_PUBLICATION_ATTEMPT_ID,
        intentId: MICRO_039_E017_PUBLICATION_INTENT_ID,
        idempotencyKey: MICRO_039_E017_PUBLICATION_IDEMPOTENCY_KEY,
        attemptFence: 1,
        binding: {
          provider: "tiktok",
          ...
          episodeId: "e017",
          episodeRevisionId: "rev.script.en-us.e017",
          locale: "en-US",
          renderHash: enEpisode.visualRenderHash,
          metadataRevisionId: "meta.rev.micro-039-e017.e017",
          consentRevisionId: "consent.micro-039-e017.e017",
          exportApprovalRevisionId: "export.approval.micro-039-e017.e017",
          privacy: "private",
          interactionSettings: {
            allowComments: false,
            allowDuet: false,
        ...
        transferPlanId: "transfer.micro-039-e017.e017",
        ...
        providerAccountId: MICRO_050_CANARY_PROVIDER_ACCOUNT_ID,
        credentialVersion: MICRO_050_REFRESHED_CREDENTIAL_VERSION_ID,
        accountFence: "fence.micro-039-e017.1",
    ...
    publishId =
      initResult.kind === "initialized"
        ...
        : null;
    receiptPublicVideoId = "video.micro-039-e017.e017.private";
  }
  ...
  const evidence = {
    schemaVersion: "mediaforge.microdrama.micro-039-e017-batch-execution-evidence.v1",
    taskId: MICRO_039_E017_TASK_ID,
  ...
  microdramaRepository.replaceProjection({
    projectionKey: MICRO_039_E017_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY,
    projection: evidence,
