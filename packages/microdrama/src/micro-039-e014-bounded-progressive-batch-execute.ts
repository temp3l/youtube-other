...
import {
  loadMicro039E014AssetGenerationApproval,
  loadMicro039E014CostBudgetApproval,
  loadMicro039E014OperatorAuthorization,
} from "./micro-039-e014-batch-authorization-persistence.js";
import {
  buildMicro039E014BatchBindingProbe,
  isMicro039E014OutOfScopeEpisodeId,
  MICRO_039_E014_BATCH_COST_LIMIT_MINOR,
  MICRO_039_E014_BATCH_EPISODE_IDS,
  MICRO_039_E014_BATCH_LOCALES,
  MICRO_039_E014_BATCH_MAXIMUM_TOTAL_PROVIDER_REQUESTS,
  MICRO_039_E014_EPISODE_RANGE,
  MICRO_039_E014_PUBLICATION_ATTEMPT_ID,
  MICRO_039_E014_PUBLICATION_IDEMPOTENCY_KEY,
  MICRO_039_E014_PUBLICATION_INTENT_ID,
  MICRO_039_E014_TASK_ID,
  type Micro039E014BatchLocale,
...
import {
  ...
  loadMicro036BatchEvidenceForMicro039E014,
  loadMicro038PublicEvidenceForMicro039E014,
  loadMicro039E013BatchEvidenceForMicro039E014,
  loadMicro042ReadEvidenceForMicro039E014,
  micro036EvidenceProvesBatchDone,
  micro038EvidenceProvesPublicDone,
  micro039E013EvidenceProvesProgressiveDone,
  micro042EvidenceProvesReadDone,
} from "./micro-039-e014-batch-evidence.js";
import {
  buildMicro039E014ExplicitExecuteAuthorizationRecord,
  computeMicro039E014PreparationFingerprint,
  loadMicro039E014ExplicitExecuteAuthorization,
  persistMicro039E014ExplicitExecuteAuthorization,
  summarizeMicro039E014PreparationForFingerprint,
} from "./micro-039-e014-explicit-execute-authorization.js";
import { evaluateMicro039E014ProgressiveBatchPreflight } from "./micro-039-e014-progressive-batch-preflight.js";

export const MICRO_039_E014_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY =
  "microdrama.batch-execution-evidence.MICRO-039-E014";

export type Micro039E014ProgressiveBatchPorts = {
...
export function createMicro039E014ProgressiveBatchPorts(): Micro039E014ProgressiveBatchPorts {
  ...
  return {
    ...
    directPostAdapter: new FixtureTikTokDirectPostAdapter("tiktok.publish.micro-039-e014"),
    auditRepository,
  };
}

export type Micro039E014EpisodeBatchEvidence = {
  readonly locale: Micro039E014BatchLocale;
  readonly episodeId: "E014";
...
export type Micro039E014AuthorizeExplicitExecuteInput = {
  ...
  readonly micro039E013EvidenceJsonPath?: string;
  readonly micro042EvidenceJsonPath?: string;
};

export type Micro039E014AuthorizeExplicitExecuteResult = {
...
export type Micro039E014BoundedProgressiveBatchExecuteInput = {
  readonly dbPath: string;
  ...
  readonly ports: Micro039E014ProgressiveBatchPorts;
...
export type Micro039E014BoundedProgressiveBatchExecuteResult = {
  ...
  readonly episodes: readonly Micro039E014EpisodeBatchEvidence[];
...
function loadUpstream(input: {
  ...
  const micro036Evidence = loadMicro036BatchEvidenceForMicro039E014({
    repository: input.repository,
    jsonFilePath:
      input.micro036EvidenceJsonPath ?? DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
  });
  const micro038Evidence = loadMicro038PublicEvidenceForMicro039E014({
    repository: input.repository,
    jsonFilePath:
      input.micro038EvidenceJsonPath ?? DEFAULT_MICRO_038_PUBLIC_EVIDENCE_JSON_PATH,
  });
  const micro039E013Evidence = loadMicro039E013BatchEvidenceForMicro039E014({
    repository: input.repository,
    jsonFilePath:
      input.micro039E013EvidenceJsonPath ??
      DEFAULT_MICRO_039_E013_BATCH_EVIDENCE_JSON_PATH,
  });
  const micro042Evidence = loadMicro042ReadEvidenceForMicro039E014({
    repository: input.repository,
    jsonFilePath:
  ...
  return {
    micro036Evidence,
    micro038Evidence,
    micro039E013Evidence,
    micro042Evidence,
  };
}

export async function authorizeMicro039E014BoundedProgressiveBatchExplicitExecute(
  input: Micro039E014AuthorizeExplicitExecuteInput
): Promise<Micro039E014AuthorizeExplicitExecuteResult> {
  ...
  const operatorAuthorization = loadMicro039E014OperatorAuthorization(microdramaRepository);
  const assetGenerationApproval =
    loadMicro039E014AssetGenerationApproval(microdramaRepository);
  const costBudgetApproval = loadMicro039E014CostBudgetApproval(microdramaRepository);
  ...
  const { micro036Evidence, micro038Evidence, micro039E013Evidence, micro042Evidence } =
    loadUpstream({
    ...
    ...(input.micro038EvidenceJsonPath
      ...
      : {}),
    ...(input.micro039E013EvidenceJsonPath
      ? { micro039E013EvidenceJsonPath: input.micro039E013EvidenceJsonPath }
      : {}),
  ...
  if (!micro039E013EvidenceProvesProgressiveDone(micro039E013Evidence)) {
    blockers.push("MICRO_039_E013_EVIDENCE_MISSING");
  }
  if (!micro042EvidenceProvesReadDone(micro042Evidence)) {
    blockers.push("MICRO_042_EVIDENCE_MISSING");
  }

  const preflight = evaluateMicro039E014ProgressiveBatchPreflight({
  ...
  const bindingProbe = buildMicro039E014BatchBindingProbe({ revisionSet });
  const preparationSummary = summarizeMicro039E014PreparationForFingerprint(
    microdramaRepository,
    {
      micro036EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e014.micro-036",
        micro036Evidence
      ),
      micro038EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e014.micro-038",
        micro038Evidence
      ),
      micro039E013EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e014.micro-039-e013",
        micro039E013Evidence
      ),
      micro042EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e014.micro-042",
        micro042Evidence
      ),
  ...
  const preparationFingerprint = computeMicro039E014PreparationFingerprint(
    preparationSummary
  );

  const record = buildMicro039E014ExplicitExecuteAuthorizationRecord({
    preparationFingerprint,
    binds: {
      episodeRange: { ...MICRO_039_E014_EPISODE_RANGE },
      locales: [...MICRO_039_E014_BATCH_LOCALES],
...
export async function executeMicro039E014BoundedProgressiveBatch(
  input: Micro039E014BoundedProgressiveBatchExecuteInput
): Promise<Micro039E014BoundedProgressiveBatchExecuteResult> {
  const blockers: string[] = [];
  const operatorId = input.operatorId ?? "operator.microdrama";
  const packRoot = input.packRoot ?? defaultV5PackRoot;
  ...
  const executeAuthorization =
    loadMicro039E014ExplicitExecuteAuthorization(microdramaRepository);
  const operatorAuthorization = loadMicro039E014OperatorAuthorization(microdramaRepository);
  const assetGenerationApproval =
    loadMicro039E014AssetGenerationApproval(microdramaRepository);
  const costBudgetApproval = loadMicro039E014CostBudgetApproval(microdramaRepository);
  ...
  for (const episodeId of input.requestedEpisodeIds ?? MICRO_039_E014_BATCH_EPISODE_IDS) {
    if (isMicro039E014OutOfScopeEpisodeId(episodeId)) {
  ...
  if (blockers.length > 0) {
    return {
      ...
      evidenceProjectionKey: MICRO_039_E014_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY,
    };
  }
  ...
  for (const profile of defaultMicro039BudgetProfiles({
    evaluatedAt: input.executedAt,
    episodeIds: MICRO_039_E014_BATCH_EPISODE_IDS,
  })) {
  ...
  const estimatedCostMinor = Math.floor(
    MICRO_039_E014_BATCH_COST_LIMIT_MINOR / MICRO_039_E014_BATCH_LOCALES.length
  );
  ...
  const episodes: Micro039E014EpisodeBatchEvidence[] = [];

  for (const episodeId of MICRO_039_E014_BATCH_EPISODE_IDS) {
    for (const locale of MICRO_039_E014_BATCH_LOCALES) {
      const produced = await produceMicro039LocaleEpisodeTts({
        taskId: MICRO_039_E014_TASK_ID,
        attributionPrefix: "micro-039-e014",
        locale,
        episodeId,
        packRoot,
        outputRoot: input.outputRoot,
        executedAt: input.executedAt,
        costLimitMinor: MICRO_039_E014_BATCH_COST_LIMIT_MINOR,
        maximumProviderRequests: MICRO_039_E014_BATCH_MAXIMUM_TOTAL_PROVIDER_REQUESTS,
        estimatedCostMinor,
      ...
      episodes.push({
        locale: locale as Micro039E014BatchLocale,
        episodeId,
  ...
  // Fixture private publication for en-US E014 only (one publication call).
  const enEpisode = episodes.find(
    (entry) => entry.locale === "en-US" && entry.episodeId === "E014"
  );
  ...
  if (enEpisode) {
    const initResult = await input.ports.directPostAdapter.initDirectPost({
      request: {
        ...
        initRequestId: "init.request.micro-039-e014.e014",
        attemptId: MICRO_039_E014_PUBLICATION_ATTEMPT_ID,
        intentId: MICRO_039_E014_PUBLICATION_INTENT_ID,
        idempotencyKey: MICRO_039_E014_PUBLICATION_IDEMPOTENCY_KEY,
        attemptFence: 1,
        binding: {
          provider: "tiktok",
          ...
          episodeId: "e014",
          episodeRevisionId: "rev.script.en-us.e014",
          locale: "en-US",
          renderHash: enEpisode.visualRenderHash,
          metadataRevisionId: "meta.rev.micro-039-e014.e014",
          consentRevisionId: "consent.micro-039-e014.e014",
          exportApprovalRevisionId: "export.approval.micro-039-e014.e014",
          privacy: "private",
          interactionSettings: {
            allowComments: false,
            allowDuet: false,
        ...
        transferPlanId: "transfer.micro-039-e014.e014",
        ...
        providerAccountId: MICRO_050_CANARY_PROVIDER_ACCOUNT_ID,
        credentialVersion: MICRO_050_REFRESHED_CREDENTIAL_VERSION_ID,
        accountFence: "fence.micro-039-e014.1",
    ...
    publishId =
      initResult.kind === "initialized"
        ...
        : null;
    receiptPublicVideoId = "video.micro-039-e014.e014.private";
  }
  ...
  const evidence = {
    schemaVersion: "mediaforge.microdrama.micro-039-e014-batch-execution-evidence.v1",
    taskId: MICRO_039_E014_TASK_ID,
  ...
  microdramaRepository.replaceProjection({
    projectionKey: MICRO_039_E014_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY,
    projection: evidence,
