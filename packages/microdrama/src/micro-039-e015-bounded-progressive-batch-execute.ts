...
import {
  ...
  isMicro039E015OutOfScopeEpisodeId,
  ...
  MICRO_039_E015_EPISODE_RANGE,
  MICRO_039_E015_PUBLICATION_ATTEMPT_ID,
  MICRO_039_E015_PUBLICATION_IDEMPOTENCY_KEY,
  MICRO_039_E015_PUBLICATION_INTENT_ID,
  MICRO_039_E015_TASK_ID,
  type Micro039E015BatchLocale,
...
import {
  buildMicro039E015ExplicitExecuteAuthorizationRecord,
  computeMicro039E015PreparationFingerprint,
  loadMicro039E015ExplicitExecuteAuthorization,
  persistMicro039E015ExplicitExecuteAuthorization,
  summarizeMicro039E015PreparationForFingerprint,
} from "./micro-039-e015-explicit-execute-authorization.js";
import { evaluateMicro039E015ProgressiveBatchPreflight } from "./micro-039-e015-progressive-batch-preflight.js";

export const MICRO_039_E015_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY =
  "microdrama.batch-execution-evidence.MICRO-039-E015";

export type Micro039E015ProgressiveBatchPorts = {
...
export function createMicro039E015ProgressiveBatchPorts(): Micro039E015ProgressiveBatchPorts {
  ...
  return {
    ...
    directPostAdapter: new FixtureTikTokDirectPostAdapter("tiktok.publish.micro-039-e015"),
    auditRepository,
  };
}

export type Micro039E015EpisodeBatchEvidence = {
  readonly locale: Micro039E015BatchLocale;
  readonly episodeId: "E015";
...
export type Micro039E015AuthorizeExplicitExecuteInput = {
...
export type Micro039E015AuthorizeExplicitExecuteResult = {
...
export type Micro039E015BoundedProgressiveBatchExecuteInput = {
  readonly dbPath: string;
  ...
  readonly ports: Micro039E015ProgressiveBatchPorts;
...
export type Micro039E015BoundedProgressiveBatchExecuteResult = {
  ...
  readonly episodes: readonly Micro039E015EpisodeBatchEvidence[];
...
export async function authorizeMicro039E015BoundedProgressiveBatchExplicitExecute(
  input: Micro039E015AuthorizeExplicitExecuteInput
): Promise<Micro039E015AuthorizeExplicitExecuteResult> {
  ...
  const { micro036Evidence, micro038Evidence, micro039E014Evidence, micro042Evidence } =
    loadUpstream({
    ...
    ...(input.micro038EvidenceJsonPath
      ...
      : {}),
    ...(input.micro039E014EvidenceJsonPath
      ? { micro039E014EvidenceJsonPath: input.micro039E014EvidenceJsonPath }
      : {}),
  ...
  const preflight = evaluateMicro039E015ProgressiveBatchPreflight({
  ...
  const bindingProbe = buildMicro039E015BatchBindingProbe({ revisionSet });
  const preparationSummary = summarizeMicro039E015PreparationForFingerprint(
    microdramaRepository,
    {
      micro036EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e015.micro-036",
        micro036Evidence
      ),
      micro038EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e015.micro-038",
        micro038Evidence
      ),
      micro039E014EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e015.micro-039-e014",
        micro039E014Evidence
      ),
      micro042EvidenceContentHash: computeUpstreamEvidenceContentHash(
        "micro-039-e015.micro-042",
        micro042Evidence
      ),
  ...
  const preparationFingerprint = computeMicro039E015PreparationFingerprint(
    preparationSummary
  );

  const record = buildMicro039E015ExplicitExecuteAuthorizationRecord({
    preparationFingerprint,
    binds: {
      episodeRange: { ...MICRO_039_E015_EPISODE_RANGE },
...
export async function executeMicro039E015BoundedProgressiveBatch(
  input: Micro039E015BoundedProgressiveBatchExecuteInput
): Promise<Micro039E015BoundedProgressiveBatchExecuteResult> {
  const blockers: string[] = [];
  const operatorId = input.operatorId ?? "operator.microdrama";
  const packRoot = input.packRoot ?? defaultV5PackRoot;
  ...
  const executeAuthorization =
    loadMicro039E015ExplicitExecuteAuthorization(microdramaRepository);
  const operatorAuthorization = loadMicro039E015OperatorAuthorization(microdramaRepository);
  const assetGenerationApproval =
    loadMicro039E015AssetGenerationApproval(microdramaRepository);
  const costBudgetApproval = loadMicro039E015CostBudgetApproval(microdramaRepository);
  ...
  for (const episodeId of input.requestedEpisodeIds ?? MICRO_039_E015_BATCH_EPISODE_IDS) {
    if (isMicro039E015OutOfScopeEpisodeId(episodeId)) {
  ...
  if (blockers.length > 0) {
    return {
      ...
      evidenceProjectionKey: MICRO_039_E015_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY,
    };
  }
  ...
  for (const profile of defaultMicro039BudgetProfiles({
    evaluatedAt: input.executedAt,
    episodeIds: MICRO_039_E015_BATCH_EPISODE_IDS,
  })) {
  ...
  const estimatedCostMinor = Math.floor(
    MICRO_039_E015_BATCH_COST_LIMIT_MINOR / MICRO_039_E015_BATCH_LOCALES.length
  );
  ...
  const episodes: Micro039E015EpisodeBatchEvidence[] = [];
  ...
  for (const episodeId of MICRO_039_E015_BATCH_EPISODE_IDS) {
    for (const locale of MICRO_039_E015_BATCH_LOCALES) {
      const produced = await produceMicro039LocaleEpisodeTts({
        ...
        attributionPrefix: "micro-039-e015",
        locale,
        episodeId,
        packRoot,
      ...
      episodes.push({
        locale: locale as Micro039E015BatchLocale,
        episodeId,
  ...
  // Fixture private publication for en-US E015 only (one publication call).
  const enEpisode = episodes.find(
    (entry) => entry.locale === "en-US" && entry.episodeId === "E015"
  );
  ...
  if (enEpisode) {
    const initResult = await input.ports.directPostAdapter.initDirectPost({
      request: {
        ...
        initRequestId: "init.request.micro-039-e015.e015",
        attemptId: MICRO_039_E015_PUBLICATION_ATTEMPT_ID,
        intentId: MICRO_039_E015_PUBLICATION_INTENT_ID,
        idempotencyKey: MICRO_039_E015_PUBLICATION_IDEMPOTENCY_KEY,
        attemptFence: 1,
        binding: {
          provider: "tiktok",
          ...
          episodeId: "e015",
          episodeRevisionId: "rev.script.en-us.e015",
          locale: "en-US",
          renderHash: enEpisode.visualRenderHash,
          metadataRevisionId: "meta.rev.micro-039-e015.e015",
          consentRevisionId: "consent.micro-039-e015.e015",
          exportApprovalRevisionId: "export.approval.micro-039-e015.e015",
          privacy: "private",
          interactionSettings: {
            allowComments: false,
            allowDuet: false,
        ...
        transferPlanId: "transfer.micro-039-e015.e015",
        ...
        providerAccountId: MICRO_050_CANARY_PROVIDER_ACCOUNT_ID,
        credentialVersion: MICRO_050_REFRESHED_CREDENTIAL_VERSION_ID,
        accountFence: "fence.micro-039-e015.1",
    ...
    publishId =
      initResult.kind === "initialized"
        ...
        : null;
    receiptPublicVideoId = "video.micro-039-e015.e015.private";
  }
  ...
  const evidence = {
    schemaVersion: "mediaforge.microdrama.micro-039-e015-batch-execution-evidence.v1",
    taskId: MICRO_039_E015_TASK_ID,
  ...
  microdramaRepository.replaceProjection({
    projectionKey: MICRO_039_E015_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY,
    projection: evidence,
