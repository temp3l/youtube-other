...
export type Micro039UpstreamEvidence = {
...
function loadJsonEvidence(path: string): Micro039UpstreamEvidence | null {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Micro039UpstreamEvidence;
...
function loadProjectionEvidence(
  ...
  key: string
): Micro039UpstreamEvidence | null {
  const stored = repository.getProjection(key);
  if (!stored) {
    return null;
  }
  const projection = stored.projection as Micro039UpstreamEvidence & {
...
export function loadMicro036BatchEvidenceForMicro039(input: {
...
export function loadMicro038PublicEvidenceForMicro039(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro042ReadEvidenceForMicro039(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039UpstreamEvidence | null {
  if (input.repository) {
...
export function micro036EvidenceProvesBatchDone(
  evidence: Micro039UpstreamEvidence | null
): boolean {
