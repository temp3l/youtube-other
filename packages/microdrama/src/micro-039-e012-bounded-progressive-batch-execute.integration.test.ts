...
import { assertMicro039E012EpisodeRangeAllowed } from "./micro-039-e012-batch-bindings.js";
import {
...
import { prepareMicro039E012BoundedProgressiveBatchAuthorization } from "./micro-039-e012-bounded-progressive-batch-authorization-preparation.js";
import {
...
describe("MICRO-039-E012 progressive E012 batch", () => {
  ...
  it("authorizes and executes separately authorized progressive E012 batch from upstream evidence", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "mediaforge-micro-039-e012-"));
    const dbPath = path.join(dir, "microdrama.sqlite");
    const outputRoot = path.join(dir, "output");

    const preparation = await prepareMicro039E012BoundedProgressiveBatchAuthorization({
      dbPath,
      ...
      micro039E011EvidenceJsonPath: DEFAULT_MICRO_039_E011_BATCH_EVIDENCE_JSON_PATH,
    ...
    const ports = createMicro039E012ProgressiveBatchPorts();
    const authorization = await authorizeMicro039E012BoundedProgressiveBatchExplicitExecute({
      dbPath,
    ...
    const execution = await executeMicro039E012BoundedProgressiveBatch({
      dbPath,
    ...
    expect(execution.episodes.every((episode) => episode.episodeId === "E012")).toBe(
      true
    );
    ...
    expect(execution.publishId).toMatch(/^tiktok\.publish\.micro-039-e012\./);
    expect(execution.observationAudit.status).toBe("PASSED");

    const blockedOutOfScope = await executeMicro039E012BoundedProgressiveBatch({
      dbPath,
      executedAt: EXECUTED_AT,
      outputRoot: path.join(dir, "blocked"),
      ports: createMicro039E012ProgressiveBatchPorts(),
      segmentSynthesisPorts: mockSegmentPorts(),
      measureDurationMs: () => 120,
      skipNarrationConcat: true,
      requestedEpisodeIds: ["E013"],
      micro036EvidenceJsonPath: DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
      micro038EvidenceJsonPath: DEFAULT_MICRO_038_PUBLIC_EVIDENCE_JSON_PATH,
      micro039E011EvidenceJsonPath: DEFAULT_MICRO_039_E011_BATCH_EVIDENCE_JSON_PATH,
      micro042EvidenceJsonPath: DEFAULT_MICRO_042_READ_EVIDENCE_JSON_PATH,
    ...
    expect(
      blockedOutOfScope.blockers.some((blocker) => blocker.includes("E013"))
    ).toBe(true);
    ...
    const stored = repo.getProjection(MICRO_039_E012_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY);
    expect(stored?.projection.publicationCalls).toBe(1);
  }, 180_000);

  it.runIf(process.env.MICRO_039_E012_OPERATOR_PREP === "1")(
    "prepares workspace operator embedded database",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath = path.join(repoRoot, ".mediaforge.sqlite");
      const preparedAt = process.env.MICRO_039_E012_PREPARED_AT ?? new Date().toISOString();

      const result = await prepareMicro039E012BoundedProgressiveBatchAuthorization({
        dbPath,
        admittedAt: ADMITTED_AT,
        preparedAt,
        micro036EvidenceJsonPath: DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e012-authorization-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e012-authorization-evidence.v1",
            taskId: "MICRO-039-E012",
            dbPath,
            preparedAt,
            ...result,
          },
  ...
  it.runIf(process.env.MICRO_039_E012_OPERATOR_EXECUTE === "1")(
    "executes workspace operator progressive E012 batch",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath =
        process.env.MICRO_039_E012_DB_PATH ?? path.join(repoRoot, ".mediaforge.sqlite");
      const executedAt = process.env.MICRO_039_E012_EXECUTED_AT ?? new Date().toISOString();
      const outputRoot = path.join(repoRoot, ".artifacts/microdrama/micro-039-e012-batch");
      const ports = createMicro039E012ProgressiveBatchPorts();
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e012-batch-execution-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e012-batch-execution-evidence.v1",
            dbPath,
            executedAt,
  ...
  it.runIf(process.env.MICRO_039_E012_LIVE_TTS === "1")(
    "executes workspace operator progressive E012 batch with live OpenAI TTS",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath =
        process.env.MICRO_039_E012_DB_PATH ?? path.join(repoRoot, ".mediaforge.sqlite");
      const executedAt =
        process.env.MICRO_039_E012_EXECUTED_AT ?? new Date().toISOString();
      const outputRoot = path.join(
        repoRoot,
        ".artifacts/microdrama/micro-039-e012-live-tts-batch"
      );
      const ports = createMicro039E012ProgressiveBatchPorts();
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e012-live-tts-batch-execution-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e012-live-tts-batch-execution-evidence.v1",
            dbPath,
            executedAt,
            outputRoot,
            publicationCalls: execution.publicationCalls,
