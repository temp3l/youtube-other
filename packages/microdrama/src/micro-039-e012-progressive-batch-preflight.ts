...
import {
  ...
  type Micro039E012UpstreamEvidence,
} from "./micro-039-e012-batch-evidence.js";
import { SEVEN_MINUTES_AHEAD_SERIES_ID } from "./v5-pack-constants.js";

export const MICRO_039_E012_LEARNING_ADMISSION_ID =
  "admission.micro-039-e012.e012.hook-presentation";

function anchoredSnapshotForE011(): NarrativeSnapshotPayload {
  return {
    schemaVersion: NARRATIVE_SCHEMA_VERSION,
    snapshotId: narrativeSnapshotIdSchema.parse("snapshot.season-1.e011"),
    seriesId: seriesIdSchema.parse(SEVEN_MINUTES_AHEAD_SERIES_ID),
    episodeId: "episode.e011",
    ...
    provenance: {
      ...
      sourceRevisionIds: [
        narrativeRevisionIdSchema.parse("rev.snapshot.season-1.e011"),
      ],
    },
  };
}

export function evaluateMicro039E012LearningAdmission(input: {
  ...
  const recommendation = planCreativeRecommendation({
    ...
    guidance:
      "Tighten the cold-open hook presentation for E012 based on public-video counters.",
    status: "VALIDATED",
    recordedAt: input.evaluatedAt,
  });

  const context = {
    anchoredSnapshot: anchoredSnapshotForE011(),
    anchoredSnapshotRevisionId: "rev.snapshot.season-1.e011",
    lastAcceptedEpisodeNumber: 11,
  };
  ...
  const acceptance = acceptCreativeRecommendationForPlanning({
    recommendation,
    context,
    admissionId: MICRO_039_E012_LEARNING_ADMISSION_ID,
...
export type Micro039E012ProgressiveBatchPreflightInput = {
  readonly evaluatedAt: string;
  ...
  readonly micro036Evidence?: Micro039E012UpstreamEvidence | null;
  readonly micro038Evidence?: Micro039E012UpstreamEvidence | null;
  readonly micro039E011Evidence?: Micro039E012UpstreamEvidence | null;
  readonly micro042Evidence?: Micro039E012UpstreamEvidence | null;
...
export type Micro039E012ProgressiveBatchPreflightResult = {
  readonly preflight: MicrodramaCanaryPreflightResult;
  readonly bindingProbe: ReturnType<typeof buildMicro039E012BatchBindingProbe>;
  readonly learningAdmissionId: string | null;
  readonly rangeAdmission: ReturnType<typeof assertMicro039E012EpisodeRangeAllowed>;
};

export function evaluateMicro039E012ProgressiveBatchPreflight(
  input: Micro039E012ProgressiveBatchPreflightInput
): Micro039E012ProgressiveBatchPreflightResult {
  const bindingProbe =
    input.operatorAuthorization &&
    "episodeRange" in input.operatorAuthorization.bindings
      ? input.operatorAuthorization.bindings
      : buildMicro039E012BatchBindingProbe({ revisionSet: input.revisionSet });

  const rangeAdmission = assertMicro039E012EpisodeRangeAllowed(bindingProbe.episodeRange);
  const learning = evaluateMicro039E012LearningAdmission({
    evaluatedAt: input.evaluatedAt,
  });

  const assetApproved =
    input.assetGenerationApproval?.state === "active" &&
    input.assetGenerationApproval.taskId === MICRO_039_E012_TASK_ID;
  ...
  const readinessGates = [
    ...
    {
      gate: "UPSTREAM_PROGRESSIVE_E011_BATCH_DONE",
      ok: micro039E011EvidenceProvesProgressiveDone(input.micro039E011Evidence ?? null),
      ...(micro039E011EvidenceProvesProgressiveDone(input.micro039E011Evidence ?? null)
        ? {}
        : { message: "MICRO-039 E011 progressive batch evidence missing." }),
    },
    {
...
export { MICRO_039_E012_EPISODE_RANGE };
