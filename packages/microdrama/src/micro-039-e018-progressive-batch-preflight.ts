...
import {
  ...
  type Micro039E018UpstreamEvidence,
} from "./micro-039-e018-batch-evidence.js";
import { SEVEN_MINUTES_AHEAD_SERIES_ID } from "./v5-pack-constants.js";

export const MICRO_039_E018_LEARNING_ADMISSION_ID =
  "admission.micro-039-e018.e018.hook-presentation";

function anchoredSnapshotForE017(): NarrativeSnapshotPayload {
  return {
    schemaVersion: NARRATIVE_SCHEMA_VERSION,
    snapshotId: narrativeSnapshotIdSchema.parse("snapshot.season-1.e017"),
    seriesId: seriesIdSchema.parse(SEVEN_MINUTES_AHEAD_SERIES_ID),
    episodeId: "episode.e017",
    ...
    provenance: {
      ...
      sourceRevisionIds: [
        narrativeRevisionIdSchema.parse("rev.snapshot.season-1.e017"),
      ],
    },
  };
}

export function evaluateMicro039E018LearningAdmission(input: {
  ...
  const recommendation = planCreativeRecommendation({
    ...
    guidance:
      "Tighten the cold-open hook presentation for E018 based on public-video counters.",
    status: "VALIDATED",
    recordedAt: input.evaluatedAt,
  });

  const context = {
    anchoredSnapshot: anchoredSnapshotForE017(),
    anchoredSnapshotRevisionId: "rev.snapshot.season-1.e017",
    lastAcceptedEpisodeNumber: 17,
  };
  ...
  const acceptance = acceptCreativeRecommendationForPlanning({
    recommendation,
    context,
    admissionId: MICRO_039_E018_LEARNING_ADMISSION_ID,
...
export type Micro039E018ProgressiveBatchPreflightInput = {
  readonly evaluatedAt: string;
  ...
  readonly micro036Evidence?: Micro039E018UpstreamEvidence | null;
  readonly micro038Evidence?: Micro039E018UpstreamEvidence | null;
  readonly micro039E017Evidence?: Micro039E018UpstreamEvidence | null;
  readonly micro042Evidence?: Micro039E018UpstreamEvidence | null;
...
export type Micro039E018ProgressiveBatchPreflightResult = {
  readonly preflight: MicrodramaCanaryPreflightResult;
  readonly bindingProbe: ReturnType<typeof buildMicro039E018BatchBindingProbe>;
  readonly learningAdmissionId: string | null;
  readonly rangeAdmission: ReturnType<typeof assertMicro039E018EpisodeRangeAllowed>;
};

export function evaluateMicro039E018ProgressiveBatchPreflight(
  input: Micro039E018ProgressiveBatchPreflightInput
): Micro039E018ProgressiveBatchPreflightResult {
  const bindingProbe =
    input.operatorAuthorization &&
    "episodeRange" in input.operatorAuthorization.bindings
      ? input.operatorAuthorization.bindings
      : buildMicro039E018BatchBindingProbe({ revisionSet: input.revisionSet });

  const rangeAdmission = assertMicro039E018EpisodeRangeAllowed(bindingProbe.episodeRange);
  const learning = evaluateMicro039E018LearningAdmission({
    evaluatedAt: input.evaluatedAt,
  });

  const assetApproved =
    input.assetGenerationApproval?.state === "active" &&
    input.assetGenerationApproval.taskId === MICRO_039_E018_TASK_ID;
  ...
  const readinessGates = [
    ...
    {
      gate: "UPSTREAM_PROGRESSIVE_E017_BATCH_DONE",
      ok: micro039E017EvidenceProvesProgressiveDone(input.micro039E017Evidence ?? null),
      ...(micro039E017EvidenceProvesProgressiveDone(input.micro039E017Evidence ?? null)
        ? {}
        : { message: "MICRO-039 E017 progressive batch evidence missing." }),
    },
    {
...
export { MICRO_039_E018_EPISODE_RANGE };
