...
import { assertMicro039E013EpisodeRangeAllowed } from "./micro-039-e013-batch-bindings.js";
import {
...
import { prepareMicro039E013BoundedProgressiveBatchAuthorization } from "./micro-039-e013-bounded-progressive-batch-authorization-preparation.js";
import {
...
describe("MICRO-039-E013 progressive E013 batch", () => {
  ...
  it("authorizes and executes separately authorized progressive E013 batch from upstream evidence", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "mediaforge-micro-039-e013-"));
    const dbPath = path.join(dir, "microdrama.sqlite");
    const outputRoot = path.join(dir, "output");

    const preparation = await prepareMicro039E013BoundedProgressiveBatchAuthorization({
      dbPath,
      ...
      micro039E012EvidenceJsonPath: DEFAULT_MICRO_039_E012_BATCH_EVIDENCE_JSON_PATH,
    ...
    const ports = createMicro039E013ProgressiveBatchPorts();
    const authorization = await authorizeMicro039E013BoundedProgressiveBatchExplicitExecute({
      dbPath,
    ...
    const execution = await executeMicro039E013BoundedProgressiveBatch({
      dbPath,
    ...
    expect(execution.episodes.every((episode) => episode.episodeId === "E013")).toBe(
      true
    );
    ...
    expect(execution.publishId).toMatch(/^tiktok\.publish\.micro-039-e013\./);
    expect(execution.observationAudit.status).toBe("PASSED");

    const blockedOutOfScope = await executeMicro039E013BoundedProgressiveBatch({
      dbPath,
      executedAt: EXECUTED_AT,
      outputRoot: path.join(dir, "blocked"),
      ports: createMicro039E013ProgressiveBatchPorts(),
      segmentSynthesisPorts: mockSegmentPorts(),
      measureDurationMs: () => 120,
      skipNarrationConcat: true,
      requestedEpisodeIds: ["E014"],
      micro036EvidenceJsonPath: DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
      micro038EvidenceJsonPath: DEFAULT_MICRO_038_PUBLIC_EVIDENCE_JSON_PATH,
      micro039E012EvidenceJsonPath: DEFAULT_MICRO_039_E012_BATCH_EVIDENCE_JSON_PATH,
      micro042EvidenceJsonPath: DEFAULT_MICRO_042_READ_EVIDENCE_JSON_PATH,
    ...
    expect(
      blockedOutOfScope.blockers.some((blocker) => blocker.includes("E014"))
    ).toBe(true);
    ...
    const stored = repo.getProjection(MICRO_039_E013_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY);
    expect(stored?.projection.publicationCalls).toBe(1);
  }, 180_000);

  it.runIf(process.env.MICRO_039_E013_OPERATOR_PREP === "1")(
    "prepares workspace operator embedded database",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath = path.join(repoRoot, ".mediaforge.sqlite");
      const preparedAt = process.env.MICRO_039_E013_PREPARED_AT ?? new Date().toISOString();

      const result = await prepareMicro039E013BoundedProgressiveBatchAuthorization({
        dbPath,
        admittedAt: ADMITTED_AT,
        preparedAt,
        micro036EvidenceJsonPath: DEFAULT_MICRO_036_BATCH_EVIDENCE_JSON_PATH,
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e013-authorization-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e013-authorization-evidence.v1",
            taskId: "MICRO-039-E013",
            dbPath,
            preparedAt,
            ...result,
          },
  ...
  it.runIf(process.env.MICRO_039_E013_OPERATOR_EXECUTE === "1")(
    "executes workspace operator progressive E013 batch",
    async () => {
      const repoRoot = path.resolve(import.meta.dirname, "../../../");
      const dbPath =
        process.env.MICRO_039_E013_DB_PATH ?? path.join(repoRoot, ".mediaforge.sqlite");
      const executedAt = process.env.MICRO_039_E013_EXECUTED_AT ?? new Date().toISOString();
      const outputRoot = path.join(repoRoot, ".artifacts/microdrama/micro-039-e013-batch");
      const ports = createMicro039E013ProgressiveBatchPorts();
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e013-batch-execution-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e013-batch-execution-evidence.v1",
            dbPath,
            executedAt,
  ...
  it.runIf(process.env.MICRO_039_E013_LIVE_TTS === "1")(
    ...
    async () => {
      ...
      const outputRoot = path.join(
        repoRoot,
        ".artifacts/microdrama/micro-039-e013-live-tts-batch"
      );
      const ports = createMicro039E013ProgressiveBatchPorts();
      ...
      writeFileSync(
        path.join(
          repoRoot,
          "docs/reports/codex-runs/2026-08-12-micro-039-e013-live-tts-batch-execution-evidence.json"
        ),
        `${JSON.stringify(
          {
            schemaVersion:
              "mediaforge.microdrama.micro-039-e013-live-tts-batch-execution-evidence.v1",
            dbPath,
            executedAt,
            outputRoot,
            publicationCalls: execution.publicationCalls,
