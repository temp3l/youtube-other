...
import { MICRO_039_E017_BATCH_EXECUTION_EVIDENCE_PROJECTION_KEY } from "./micro-039-e017-bounded-progressive-batch-execute.js";
...
export const DEFAULT_MICRO_039_E017_BATCH_EVIDENCE_JSON_PATH =
...
export type Micro039E018UpstreamEvidence = {
...
function loadJsonEvidence(path: string): Micro039E018UpstreamEvidence | null {
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8")) as Micro039E018UpstreamEvidence;
...
function loadProjectionEvidence(
  ...
  key: string
): Micro039E018UpstreamEvidence | null {
  const stored = repository.getProjection(key);
  if (!stored) {
    return null;
  }
  const projection = stored.projection as Micro039E018UpstreamEvidence & {
...
export function loadMicro036BatchEvidenceForMicro039E018(input: {
...
export function loadMicro038PublicEvidenceForMicro039E018(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E018UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro039E017BatchEvidenceForMicro039E018(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E018UpstreamEvidence | null {
  if (input.repository) {
...
export function loadMicro042ReadEvidenceForMicro039E018(input: {
  readonly repository?: MicrodramaSQLiteRepository;
  readonly jsonFilePath?: string;
}): Micro039E018UpstreamEvidence | null {
  if (input.repository) {
...
export function micro036EvidenceProvesBatchDone(
  evidence: Micro039E018UpstreamEvidence | null
): boolean {
...
export function micro039E017EvidenceProvesProgressiveDone(
  evidence: Micro039E018UpstreamEvidence | null
): boolean {
  ...
  const hasE017 = episodes.some((entry) => {
    const episodeId =
      typeof entry === "object" &&
      entry !== null &&
      "episodeId" in entry &&
      typeof (entry as { episodeId?: unknown }).episodeId === "string"
        ...
        : null;
    return episodeId === "E017";
  });
  const rangeIsE017 =
    evidence.episodeRange?.startEpisodeId?.toLowerCase() === "e017" &&
    evidence.episodeRange?.endEpisodeId?.toLowerCase() === "e017";
  return hasE017 || rangeIsE017 || (evidence.publicationCalls ?? 0) === 1;
}
