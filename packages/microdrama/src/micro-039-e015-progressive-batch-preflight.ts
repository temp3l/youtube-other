...
import {
  ...
  type Micro039E015UpstreamEvidence,
} from "./micro-039-e015-batch-evidence.js";
import { SEVEN_MINUTES_AHEAD_SERIES_ID } from "./v5-pack-constants.js";

export const MICRO_039_E015_LEARNING_ADMISSION_ID =
  "admission.micro-039-e015.e015.hook-presentation";

function anchoredSnapshotForE014(): NarrativeSnapshotPayload {
  return {
    schemaVersion: NARRATIVE_SCHEMA_VERSION,
    snapshotId: narrativeSnapshotIdSchema.parse("snapshot.season-1.e014"),
    seriesId: seriesIdSchema.parse(SEVEN_MINUTES_AHEAD_SERIES_ID),
    episodeId: "episode.e014",
    ...
    provenance: {
      ...
      sourceRevisionIds: [
        narrativeRevisionIdSchema.parse("rev.snapshot.season-1.e014"),
      ],
    },
  };
}

export function evaluateMicro039E015LearningAdmission(input: {
  ...
  const recommendation = planCreativeRecommendation({
    ...
    guidance:
      "Tighten the cold-open hook presentation for E015 based on public-video counters.",
    status: "VALIDATED",
    recordedAt: input.evaluatedAt,
  });

  const context = {
    anchoredSnapshot: anchoredSnapshotForE014(),
    anchoredSnapshotRevisionId: "rev.snapshot.season-1.e014",
    lastAcceptedEpisodeNumber: 14,
  };
  ...
  const acceptance = acceptCreativeRecommendationForPlanning({
    recommendation,
    context,
    admissionId: MICRO_039_E015_LEARNING_ADMISSION_ID,
...
export type Micro039E015ProgressiveBatchPreflightInput = {
  readonly evaluatedAt: string;
  ...
  readonly micro036Evidence?: Micro039E015UpstreamEvidence | null;
  readonly micro038Evidence?: Micro039E015UpstreamEvidence | null;
  readonly micro039E014Evidence?: Micro039E015UpstreamEvidence | null;
  readonly micro042Evidence?: Micro039E015UpstreamEvidence | null;
...
export type Micro039E015ProgressiveBatchPreflightResult = {
  readonly preflight: MicrodramaCanaryPreflightResult;
  readonly bindingProbe: ReturnType<typeof buildMicro039E015BatchBindingProbe>;
  readonly learningAdmissionId: string | null;
  readonly rangeAdmission: ReturnType<typeof assertMicro039E015EpisodeRangeAllowed>;
};

export function evaluateMicro039E015ProgressiveBatchPreflight(
  input: Micro039E015ProgressiveBatchPreflightInput
): Micro039E015ProgressiveBatchPreflightResult {
  const bindingProbe =
    input.operatorAuthorization &&
    "episodeRange" in input.operatorAuthorization.bindings
      ? input.operatorAuthorization.bindings
      : buildMicro039E015BatchBindingProbe({ revisionSet: input.revisionSet });

  const rangeAdmission = assertMicro039E015EpisodeRangeAllowed(bindingProbe.episodeRange);
  const learning = evaluateMicro039E015LearningAdmission({
    evaluatedAt: input.evaluatedAt,
  });

  const assetApproved =
    input.assetGenerationApproval?.state === "active" &&
    input.assetGenerationApproval.taskId === MICRO_039_E015_TASK_ID;
  ...
  const readinessGates = [
    ...
    {
      gate: "UPSTREAM_PROGRESSIVE_E014_BATCH_DONE",
      ok: micro039E014EvidenceProvesProgressiveDone(input.micro039E014Evidence ?? null),
      ...(micro039E014EvidenceProvesProgressiveDone(input.micro039E014Evidence ?? null)
        ? {}
        : { message: "MICRO-039 E014 progressive batch evidence missing." }),
    },
    {
...
export { MICRO_039_E015_EPISODE_RANGE };
