...
Configuration is resolved from CLI flags where available, then runtime config loaded from `.env` and process environment, then code defaults. The current cost-safe defaults are `gpt-5.4-medium` for story/localization/short, story reasoning `medium`, localization/short/validator/metadata reasoning `low`, story max-output-tokens `5500`, localization `5200`, short `1200`, validator `2500`, and metadata `1200`. User overrides remain authoritative. CLI story commands surface warning-only guidance when short caps exceed `2000`, validator caps exceed `3000`, localization uses the same model as story generation, or story caps exceed `7000` for sub-`2000`-word targets.
...
Retry and repair behavior is implemented in the services. Full localization can retry with configured retry max-output tokens and validator/repair settings. Short rewrite now persists compact `story-facts.json`, validates against deterministic quality gates, applies deterministic cleanup where possible, and allows one targeted repair attempt before rejection. Ordinary short quality failures no longer fall through to full regeneration; regeneration remains reserved for token-exhaustion retry behavior.
