...
Later reviewed direct rules remain equally bounded. In particular, `atomic-located-in-event-location-candidate.v1` admits only the exact native D-Day lineage whose subject has explicit typed event eligibility. It preserves `intended` modality and the ordered `event -> location` roles, and records a separate candidate evidence fingerprint over claim, structured-proposition, atomic-grounding, and exact source-span/hash provenance. Other `located-in` atoms—including the 1066 army-in-England control—fail closed and create no relation.

## Contract and provenance versioning

The hardened persisted relation contract is `history-explanatory-relations.v4`. V3 added kind-specific causal and policy-response modality; V4 adds only `event-location`, with an explicitly typed event/action/operation subject, a canonical place, and required assertion modality. V2/V3 artifacts remain parseable under their original restrictions, and explicit non-default legacy modality retains its prior semantic identity behavior. V1 review artifacts must not be treated as V2/V3/V4 records because V1 overloaded semantic identity with evidence provenance. `explanatoryRelationSchemaV36` is the authoritative Zod runtime validator; `relation-schema.json` is machine-enforcing Draft 2020-12 JSON Schema generated directly from it with Zod's native JSON-Schema exporter. `relation-contract-document.json` is supplemental field-level review documentation generated alongside it.

`event-location` is directed `event -> location` and never implies movement, comparison, or causality. Its assertion status always enters semantic identity. Event eligibility is explicit typed metadata (`event`, `action`, or `operation`), never a label regex; the first approved eligibility record is bound to the exact D-Day claim, structured proposition, atomic grounding, subject ID, and source hash.
