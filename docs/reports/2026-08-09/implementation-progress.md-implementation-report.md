...
Risks/next: deployment must inject `recordRecentAuth` into OIDC and `recentAuthConsumer` into runtime. Browser-flow HTTP tests remain blocked by sandbox loopback `EPERM`. YSAAS-009 now persists bounded preflight evidence and exposes an idempotent server-authoritative preflight/read API; quota reservation, child workflow admission, retry/cancel, and BFF remain outstanding.
