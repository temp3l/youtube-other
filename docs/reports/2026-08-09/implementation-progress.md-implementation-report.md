...
Risks/next: deployment must inject `recordRecentAuth` into OIDC and `recentAuthConsumer` into runtime. Browser-flow HTTP tests remain blocked by sandbox loopback `EPERM`. YSAAS-009 now has persisted preflight, authoritative API/SDK reads and preflight, quota-backed launch, worker-safe claims, child admission bindings, safe unadmitted-item cancellation, atomic terminal worker settlement, terminal capacity release, retry generations, and queued-child cancellation settlement; retry API/SDK, child cancellation fan-out, and BFF remain outstanding.
