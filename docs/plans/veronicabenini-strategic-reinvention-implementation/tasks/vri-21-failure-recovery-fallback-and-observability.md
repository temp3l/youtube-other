# VRI-21 — Failure recovery, fallback, and observability

## Objective

Standardize failure taxonomy, retry/fallback, correlation, redaction, and metrics.
...
VER-130 (PARTIAL), VER-131 (COMPLETE), VER-132 (PARTIAL), VER-133 (ABSENT), VER-134 (PARTIAL)
...
packages/application/src/durable-job-worker.ts; packages/observability/src/telemetry.ts; packages/speech/src/platform; apps/api/src/tenant-reconciliation-scheduler.ts
...
**EXCLUSIVE_WRITE_OWNER:** observability metrics/process roots. Other tasks must use additive/disjoint files or wait for this owner.
