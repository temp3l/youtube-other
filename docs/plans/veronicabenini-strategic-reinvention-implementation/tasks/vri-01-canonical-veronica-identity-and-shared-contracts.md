# VRI-01 — Canonical Veronica identity and shared contracts

## Objective

Persist veronicabenini as canonical and resolve strategic-reinvention only as a compatibility alias.
...
`CONTENT_PROFILE_IDS` and strategic profile schemas currently persist the alias.
...
packages/domain/src/workflow-contracts.ts; packages/domain/src/content-policy-contracts.ts; packages/config; packages/strategic-reinvention/src/profile.ts
...
**EXCLUSIVE_WRITE_OWNER:** shared workflow/profile schemas. Other tasks must use additive/disjoint files or wait for this owner.
