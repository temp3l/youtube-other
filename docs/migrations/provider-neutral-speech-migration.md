...
The PostgreSQL migration is additive, idempotent, and transactional. It creates tenant-RLS consent,
...
API startup runs the migration and the OpenAI system-profile backfill is safe to repeat.
...
Set the workspace dispatch control to disabled for logical rollback. Physical schema
rollback is intentionally deferred until retention and audit obligations
