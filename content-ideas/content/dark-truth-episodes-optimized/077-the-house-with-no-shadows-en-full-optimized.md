# Episode 077 — Nothing Cast a Shadow Inside the House
...
After sunset, the furniture stopped casting shadows. Then Lukas's own shadow disappeared.

Lukas Reed was a real-estate assessor when the account began in an isolated manor. The first event
...
The warning was already present: Do not enter the basement without carrying two light sources. Lukas
...
Photographs showed dark figures standing where shadows should have been. Lukas repeated the
...
The house grew colder each time another shadow vanished. The recurring sounds—light-switch clicks,
empty room tone and soft footsteps—returned only when the pattern advanced. They remained quiet
enough to ignore, but consistent enough for the audience to recognise before the characters did.

The behaviour followed one rule: The manor separated people from their shadows, then used the
shadows to imitate former residents. Once Lukas understood that rule, the earlier incidents stopped
looking random. They formed a deliberate sequence that reacted to every attempt at control.

Lukas began an investigation. Archived records, old witness statements, maintenance logs and
...
Every previous owner had left without ever being seen again. This was the first detail that survived
...
The situation became personal after that. Lukas found a basement filled with moving human
silhouettes. Until then, Lukas had been protecting a job, a record or a piece of evidence. Now the
...
Lukas considered leaving immediately, but the records suggested the pattern would follow. Earlier
...
The only workable plan was this: He used opposing floodlights to force each shadow to overlap with
its owner. The plan depended on observation rather than force. The goal was not to destroy something
that ignored ordinary limits. It was to force the pattern into repeating the one behaviour it could
not change.
...
One stable detail remained useful: light-switch clicks, empty room tone and soft footsteps. Lukas
...
check notes once the environment began changing. Lukas reduced the plan to one rule and one action,
then repeated both until hesitation was no longer possible.

Before the final attempt, Lukas waited through one complete cycle without intervening. That pause
...
The basement figures copied missing residents and tried to lead Lukas toward the wrong silhouette.
He identified his own by a scar-shaped gap in its hand. Lukas had one opportunity to commit to the
...
His shadow returned before dawn. Days later, it began moving one second before he did.
